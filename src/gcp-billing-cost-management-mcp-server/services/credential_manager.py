...
class CredentialManager:
    ...
    def validate_credentials(
        ...
        try:
            ...
            if error_code == "InvalidClientTokenId":
                ...
                logger.error(f"❌ 凭证验证失败: {error_code} - {error_msg}")
                return {"valid": False, "error": f"{error_code}: {error_msg}"}

        except Exception as e:
            logger.error(f"❌ 凭证验证失败: 未知错误 - {str(e)}")
    ...
    def create_session(
        ...
        try:
            ...
            logger.debug(f"✅ Session 创建成功 - Region: {region}")
            return session
        except Exception as e:
            logger.error(f"❌ Session 创建失败: {e}")
