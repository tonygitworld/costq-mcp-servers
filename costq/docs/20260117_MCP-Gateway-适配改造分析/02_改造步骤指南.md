# MCP Server 多账号改造步骤指南

> 本文档提供**详细的、可执行的**改造步骤，基于 **cloudtrail-mcp-server** 的实际改造方案。

---

## 🎯 快速参考：哪些文件可以直接复制？

| 文件/目录 | 是否可复制 | 需要修改 | 说明 |
|-----------|-----------|---------|------|
| `cred_extract_services/` | ✅ **可直接复制** | ❌ 无需修改 | 完全通用，自包含，不依赖项目代码 |
| `Dockerfile-AgentCore-Runtime` | ✅ **可直接复制** | ⚠️ 修改 CMD | 仅需修改启动命令中的包名 |
| `server.py` 中的 `_setup_account_context()` | ✅ **可直接复制** | ❌ 无需修改 | 统一账号上下文设置函数 |
| `costq/scripts/build_*.sh` | ⚠️ **需要修改** | ✅ 修改 1 处 | 仅需修改 `MCP_SERVER_NAME` 变量 |

**总结**：
- ✅ **90% 的文件可以直接复制**（cred_extract_services/ + Dockerfile + _setup_account_context）
- ⚠️ **10% 需要简单修改**（Dockerfile CMD、部署脚本、FastMCP 配置）
- 🚀 **改造速度极快**：预计 15-20 分钟完成基础改造

---

## 📋 改造前准备

### 1. 确认改造范围
- [ ] 确认 MCP Server 名称和路径
- [ ] 确认是否需要多账号访问能力
- [ ] 备份原始代码（git commit）

### 2. 环境准备
```bash
# 1. 安装依赖
cd src/<mcp-server-name>
pip install sqlalchemy cryptography psycopg2-binary

# 2. 配置环境变量（开发环境）
cat > .env << EOF
DATABASE_URL=postgresql://user:pass@localhost:5432/costq
ENCRYPTION_KEY=<base64-encoded-fernet-key>
AWS_REGION=us-east-1
EOF
```

---

## 🔧 改造步骤

### Step 1: 复制凭证提取服务层（✅ 可直接复制）

#### 1.1 创建目录并复制
```bash
cd src/<mcp-server-name>/

# ✅ 直接复制整个目录（完全通用，无需修改）
cp -r ../cloudtrail-mcp-server/cred_extract_services ./
```

**为什么可以直接复制？**
- ✅ 完全自包含，不依赖项目特定代码
- ✅ 没有硬编码的项目名称
- ✅ 所有配置从环境变量读取
- ✅ 可以在任何 MCP Server 中直接使用

**包含的文件**：
```
cred_extract_services/
├── __init__.py           # 公共接口导出
├── aws_client.py         # STS AssumeRole（通用）
├── context_manager.py    # 环境变量管理（通用）
├── credential_extractor.py  # 凭证提取核心（通用）
├── crypto.py             # AKSK 解密（通用）
├── database.py           # 数据库查询（通用）
└── exceptions.py         # 自定义异常（通用）
```

**验证**：
```bash
# 测试导入
python -c "from cred_extract_services import extract_aws_credentials; print('✅ 导入成功')"
```

---

### Step 2: 修改 server.py

#### 2.1 添加 FastMCP 配置参数

**修改 FastMCP 初始化**：
```python
# 原始代码
from mcp.server.fastmcp import FastMCP

mcp = FastMCP(
    'awslabs.your-mcp-server',
    instructions='...',
    dependencies=[...]
)
```

**改造后**：
```python
from mcp.server.fastmcp import FastMCP

# Create FastMCP server for AgentCore Runtime
# AgentCore requires stateless HTTP servers on 0.0.0.0:8000/mcp
mcp = FastMCP(
    name='awslabs.your-mcp-server',  # ✅ 使用 name= 参数
    instructions='...',
    dependencies=[
        'boto3',
        'botocore',
        'pydantic',
    ],
    host="0.0.0.0",           # ✅ 新增：监听地址
    stateless_http=True       # ✅ 新增：无状态 HTTP 模式
)
```

---

#### 2.2 添加导入和异常类导出

**在文件顶部添加导入**：
```python
import logging

from awslabs.your_mcp_server.tools import YourTools
from mcp.server.fastmcp import FastMCP

# ✅ 新增：导入凭证服务
from cred_extract_services.context_manager import set_aws_credentials
from cred_extract_services.credential_extractor import extract_aws_credentials
from cred_extract_services.exceptions import (
    AccountNotFoundError,
    AssumeRoleError,
    CredentialDecryptionError,
    CredentialExtractionError,
    DatabaseConnectionError,
)

logger = logging.getLogger(__name__)

# ✅ 新增：导出异常类和函数
__all__ = [
    "_setup_account_context",
    "mcp",
    "main",
    "CredentialExtractionError",
    "AccountNotFoundError",
    "CredentialDecryptionError",
    "AssumeRoleError",
    "DatabaseConnectionError",
]
```

---

#### 2.3 添加 _setup_account_context 函数

**⚠️ 重要说明：直接添加到 server.py 文件中**

**位置：在 mcp 初始化之后、main() 之前**

**为什么不创建独立模块？**
- ✅ 符合 FastMCP 架构规范（server.py 负责公共函数）
- ✅ 导入路径更简单清晰
- ✅ 这个函数可以 **100% 从 CloudTrail MCP 复制**，无需修改任何代码
- ❌ 创建独立模块会增加复杂度，降低可复用性

**复制提示：** 这个函数可以直接从 `cloudtrail-mcp-server/awslabs/cloudtrail_mcp_server/server.py` 中复制，粘贴到你的 `server.py` 中，**完全不需要修改**！

---

**完整代码如下**：

```python
async def _setup_account_context(
    target_account_id: str,
) -> dict[str, str]:
    """设置 AWS 凭证上下文

    统一入口函数，完成以下操作：
    1. 查询账号信息（自包含数据库查询）
    2. 提取凭证（AKSK 解密 / IAM Role AssumeRole）
    3. 设置环境变量

    Args:
        target_account_id: AWS 账号 ID

    Returns:
        凭证信息字典（已脱敏）

    Raises:
        AccountNotFoundError: 账号不存在
        CredentialDecryptionError: AKSK 解密失败
        AssumeRoleError: IAM Role AssumeRole 失败
        DatabaseConnectionError: 数据库连接失败
    """
    logger.info("开始设置 AWS 凭证上下文")

    # 1. 提取凭证
    credentials = await extract_aws_credentials(target_account_id)

    # 2. 设置环境变量
    set_aws_credentials(
        access_key_id=credentials["access_key_id"],
        secret_access_key=credentials["secret_access_key"],
        session_token=credentials.get("session_token"),
        region=credentials["region"],
    )

    # 3. 返回脱敏信息
    cred_info = {
        "account_id": credentials["account_id"],
        "account_alias": credentials.get("alias", "Unknown"),
        "auth_type": credentials["auth_type"],
        "region": credentials["region"],
    }

    logger.info(f"✅ AWS 凭证上下文设置完成: {cred_info}")
    return cred_info
```

**💡 这个函数可以直接从 cloudtrail-mcp-server 复制，无需修改！**

---

#### 2.3.1 完整的 server.py 结构示例

**改造后的 server.py 完整结构**：

```python
# awslabs/your_mcp_server/server.py

import logging
from mcp.server.fastmcp import FastMCP

# ✅ 导入凭证服务
from cred_extract_services.context_manager import set_aws_credentials
from cred_extract_services.credential_extractor import extract_aws_credentials
from cred_extract_services.exceptions import (
    AccountNotFoundError,
    AssumeRoleError,
    CredentialDecryptionError,
    CredentialExtractionError,
    DatabaseConnectionError,
)

logger = logging.getLogger(__name__)

# ✅ 导出异常类和函数（供 tools 使用）
__all__ = [
    "_setup_account_context",
    "mcp",
    "main",
    "CredentialExtractionError",
    "AccountNotFoundError",
    "CredentialDecryptionError",
    "AssumeRoleError",
    "DatabaseConnectionError",
]

# ===== 第 1 部分：FastMCP 初始化 =====
mcp = FastMCP(
    name='awslabs.your-mcp-server',
    instructions='...',
    dependencies=[
        'boto3',
        'botocore',
        'pydantic',
    ],
    host="0.0.0.0",           # ✅ 必须！
    stateless_http=True       # ✅ 必须！
)

# ===== 第 2 部分：账号上下文函数（✅ 100% 可复制）=====
async def _setup_account_context(
    target_account_id: str,
) -> dict[str, str]:
    """设置 AWS 凭证上下文

    ⚠️ 这个函数可以从 cloudtrail-mcp-server 直接复制，完全不需要修改！
    """
    logger.info("开始设置 AWS 凭证上下文")

    # 1. 提取凭证
    credentials = await extract_aws_credentials(target_account_id)

    # 2. 设置环境变量
    set_aws_credentials(
        access_key_id=credentials["access_key_id"],
        secret_access_key=credentials["secret_access_key"],
        session_token=credentials.get("session_token"),
        region=credentials["region"],
    )

    # 3. 返回脱敏信息
    cred_info = {
        "account_id": credentials["account_id"],
        "account_alias": credentials.get("alias", "Unknown"),
        "auth_type": credentials["auth_type"],
        "region": credentials["region"],
    }

    logger.info(f"✅ AWS 凭证上下文设置完成: {cred_info}")
    return cred_info

# ===== 第 3 部分：主入口 =====
def main():
    """Run the MCP server with streamable HTTP transport for AgentCore."""
    mcp.run(transport="streamable-http")

if __name__ == '__main__':
    main()
```

**关键点**：
1. ✅ `_setup_account_context()` 函数**直接放在 server.py 中**
2. ✅ 位置在 FastMCP 初始化**之后**、main() **之前**
3. ✅ 通过 `__all__` 导出，供 tools 文件导入使用
4. ✅ **不需要创建独立的 account_context.py 文件**

---

#### 2.4 修改 main() 函数

```python
# 原始代码
def main():
    mcp.run()
```

**改造后**：
```python
def main():
    """Run the MCP server with streamable HTTP transport for AgentCore."""
    # AgentCore Runtime expects servers to run with streamable-http transport
    mcp.run(transport="streamable-http")

if __name__ == '__main__':
    main()
```

---

#### 2.5 验证 server.py 修改

```bash
# 检查 FastMCP 配置
grep -A 5 "FastMCP(" awslabs/*/server.py

# 检查 _setup_account_context 函数
grep -A 3 "_setup_account_context" awslabs/*/server.py

# 检查 main() 函数
grep -A 3 "def main" awslabs/*/server.py
```

---

### Step 3: 修改所有 Tool 函数

#### 3.1 定位所有 Tool 文件
```bash
# 查找所有 tool 文件
find awslabs -name "*_tools.py" -type f
```

#### 3.2 参数顺序规则 ⚠️ 重要

**🎯 核心规则：`target_account_id` 必须放在所有必需参数之后、其他可选参数之前**

```python
# ✅ 正确的参数顺序
async def tool_function(
    ctx: Context,                             # 1. MCP Context（必需）
    required_param: str,                      # 2. 业务必需参数
    target_account_id: Optional[str] = None,  # 3. ⭐ target_account_id（第一个可选参数）
    optional_param: Optional[int] = None,     # 4. 其他可选参数
):

# ❌ 错误示例 1：target_account_id 在必需参数之前
async def tool_function(
    ctx: Context,
    target_account_id: Optional[str] = None,  # ❌ 错误！
    required_param: str,                      # Python 不允许必需参数在可选参数后面
):

# ❌ 错误示例 2：target_account_id 在其他可选参数之后
async def tool_function(
    ctx: Context,
    required_param: str,
    optional_param: Optional[int] = None,
    target_account_id: Optional[str] = None,  # ❌ 不规范，应该放在第一个
):
```

---

#### 3.3 标准改造模板

**改造前**：
```python
@server.tool(name="tool-name", description="...")
async def tool_function(
    ctx: Context,
    param1: str,                     # 必需参数
    param2: Optional[int] = None,    # 可选参数
) -> Dict[str, Any]:
    try:
        # 业务逻辑
        client = create_aws_client("service", "region")
        response = client.some_api_call(param1=param1)
        return format_response("success", response)
    except Exception as e:
        return await handle_aws_error(ctx, e, "operation", "Service")
```

**改造后**：
```python
# 1. 导入异常类（文件顶部）
from awslabs.your_mcp_server.server import (
    _setup_account_context,
    AccountNotFoundError,
    CredentialDecryptionError,
    AssumeRoleError,
    DatabaseConnectionError,
)

@server.tool(name="tool-name", description="...")
async def tool_function(
    ctx: Context,
    param1: str,                              # 必需参数（保持原位置）
    target_account_id: Optional[str] = None,  # ✅ 新增：第一个可选参数
    param2: Optional[int] = None,             # 可选参数（保持原位置）
) -> Dict[str, Any]:
    try:
        # ===== 账号上下文初始化 =====
        if target_account_id:
            await _setup_account_context(target_account_id)

        # ===== 原有逻辑（完全不变）=====
        client = create_aws_client("service", "region")
        response = client.some_api_call(param1=param1)
        return format_response("success", response)

    # ===== 异常处理 =====
    except AccountNotFoundError:
        return format_response('error', {'error_type': 'account_not_found'},
                               'Account not found. Please check the account ID.')
    except CredentialDecryptionError:
        return format_response('error', {'error_type': 'credential_error'},
                               'Failed to decrypt credentials. Please contact administrator.')
    except AssumeRoleError:
        return format_response('error', {'error_type': 'assume_role_error'},
                               'Failed to assume role. Please check IAM role configuration.')
    except DatabaseConnectionError:
        return format_response('error', {'error_type': 'database_error'},
                               'Database connection failed. Please try again later.')
    except Exception as e:
        return await handle_aws_error(ctx, e, "operation", "Service")
```

---

#### 3.4 批量修改辅助脚本（仅供参考）

**⚠️ 警告**：自动化脚本可能无法处理所有边界情况，**必须人工审查每个修改**

```python
# scripts/add_target_account_id.py
import re
import sys
from pathlib import Path

def add_target_account_id_param(file_path: Path, package_name: str):
    """给 tool 函数添加 target_account_id 参数"""
    content = file_path.read_text()

    # 1. 添加导入（如果不存在）
    import_block = f"""
from awslabs.{package_name}.server import (
    _setup_account_context,
    AccountNotFoundError,
    CredentialDecryptionError,
    AssumeRoleError,
    DatabaseConnectionError,
)
"""
    if "_setup_account_context" not in content:
        # 在最后一个 import 后添加
        lines = content.split('\n')
        last_import_idx = 0
        for i, line in enumerate(lines):
            if line.startswith('import ') or line.startswith('from '):
                last_import_idx = i
        lines.insert(last_import_idx + 1, import_block)
        content = '\n'.join(lines)

    # 2. 修改函数签名（添加 target_account_id）
    # 模式：async def function_name(ctx: Context, ...
    # 注意：这只是简单示例，实际需要更复杂的逻辑
    pattern = r'(async def \w+\(\s*ctx:\s*Context,)'
    replacement = r'\1\n    target_account_id: Optional[str] = None,'
    content = re.sub(pattern, replacement, content)

    # 3. 添加账号上下文初始化代码
    context_init = """
        # ===== Account context initialization =====
        if target_account_id:
            await _setup_account_context(target_account_id)

        # ===== Original logic (unchanged) =====
"""
    content = content.replace("    try:\n", f"    try:\n{context_init}")

    # 保存修改
    file_path.write_text(content)
    print(f"✅ Modified: {file_path}")


if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python add_target_account_id.py <package_name> <tool_file.py>")
        sys.exit(1)

    package_name = sys.argv[1]
    for file_path in sys.argv[2:]:
        add_target_account_id_param(Path(file_path), package_name)
```

**使用脚本**：
```bash
# 修改单个文件
python scripts/add_target_account_id.py your_mcp_server awslabs/.../tools/example_tools.py

# 批量修改（谨慎使用）
find awslabs -name "*_tools.py" -exec python scripts/add_target_account_id.py your_mcp_server {} \;
```

**⚠️ 建议**：
- 先在测试文件上验证
- **必须人工审查每个修改**
- 建议手动修改（更安全）

---

### Step 4: 更新 Tool 描述文档

#### 4.1 添加参数说明
在每个 tool 的 `description` 中添加：
```python
@server.tool(
    name="tool-name",
    description="""Tool description...

    Args:
        ctx: The MCP context object
        target_account_id: Target AWS account ID. If not provided, use default credentials.
        # ... 其他参数

    Returns:
        Dict containing the response data
    """
)
```

#### 4.2 添加使用示例
```markdown
## 使用示例

### 1. 使用默认凭证
```json
{
  "tool": "tool-name",
  "params": {
    "param1": "value1"
  }
}
```

### 2. 使用目标账号凭证
```json
{
  "tool": "tool-name",
  "params": {
    "target_account_id": "123456789012",
    "param1": "value1"
  }
}
```
```

---

### Step 4.5: 检查并统一日志库（重要）⚠️

#### 4.5.1 为什么需要统一日志库？

**问题**：混用 `loguru` 和 `logging` 会导致：
- ❌ **OpenTelemetry 无法捕获 loguru 日志**
- ❌ **缺少 trace ID 和 span ID**（无法追踪请求链路）
- ❌ **CloudWatch 日志不完整**（loguru 的日志只通过 stderr 输出，没有结构化信息）
- ❌ **日志格式不一致**（难以统一分析）

**检查方法**：
```bash
# 查找项目中是否使用 loguru
cd src/<mcp-server-name>/
grep -r "from loguru" awslabs/ --include="*.py"

# 如果找到，需要迁移到 logging
```

---

#### 4.5.2 迁移步骤

**Step 1: 修改导入语句**
```python
# 修改前（tools.py 或其他文件）
from loguru import logger

# 修改后
import logging
logger = logging.getLogger(__name__)
```

**Step 2: 移除 loguru 依赖**
```toml
# pyproject.toml
[project]
dependencies = [
    "boto3>=1.38.22",
    # "loguru>=0.7.0",  # ❌ 删除此行
    "mcp[cli]>=1.23.0",
    "pydantic>=2.10.6",
]
```

**Step 3: 更新 FastMCP 依赖声明**（如果有）
```python
# server.py
mcp = FastMCP(
    name='awslabs.xxx-mcp-server',
    dependencies=[
        'boto3',
        'botocore',
        'pydantic',
        # 'loguru',  # ❌ 删除此行
    ],
    ...
)
```

**Step 4: 验证修改**
```bash
# 确认没有 loguru 引用
grep -r "loguru" . --include="*.py" --include="*.toml" || echo "✅ 没有找到 loguru 引用"

# 语法检查
python3 -m py_compile awslabs/**/*.py
```

---

#### 4.5.3 日志方法兼容性

**好消息**：`logging` 和 `loguru` 的基础 API 完全兼容，无需修改日志调用代码！

```python
# 以下代码在两个库中都可以正常工作
logger.info(f"消息: {变量}")
logger.error(f"错误: {str(e)}")
logger.warning(f"警告: {内容}")
logger.debug(f"调试: {数据}")
```

**注意**：如果使用了 loguru 特有功能，需要调整：
```python
# ❌ loguru 特有语法（需要改）
logger.opt(exception=True).error("错误")
logger.bind(request_id=123).info("消息")
logger.catch()(my_function)

# ✅ logging 等效写法
logger.error("错误", exc_info=True)
logger.info("消息", extra={"request_id": 123})
# 使用标准异常处理替代 @logger.catch()
```

---

#### 4.5.4 迁移后的优势

| 改进项 | loguru | logging | 提升 |
|--------|--------|---------|------|
| **OpenTelemetry 集成** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **Trace ID** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **Span ID** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **CloudWatch 完整性** | ⚠️ 部分 | ✅ 完整 | ⭐⭐⭐⭐ |
| **请求链路追踪** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **日志系统统一** | ❌ | ✅ | ⭐⭐⭐⭐ |

**CloudWatch 日志对比**：
```json
// ❌ loguru（缺少追踪信息）
"2026-01-19 12:00:00.123 | INFO | tools:lookup_events:180 - 开始查询"

// ✅ logging + OpenTelemetry（完整追踪）
{
  "timestamp": "2026-01-19T12:00:00.123Z",
  "level": "INFO",
  "logger": "awslabs.xxx_mcp_server.tools",
  "message": "开始查询",
  "trace_id": "1-abc123",  // ✅ 可追踪请求
  "span_id": "xyz789"      // ✅ 可追踪函数调用
}
```

---

#### 4.5.5 快速迁移检查清单

- [ ] 查找所有 `from loguru import` 引用
- [ ] 替换为 `import logging` + `logger = logging.getLogger(__name__)`
- [ ] 从 `pyproject.toml` 移除 `loguru` 依赖
- [ ] 从 `server.py` FastMCP 依赖移除 `loguru`
- [ ] 检查是否使用了 loguru 特有语法（需要调整）
- [ ] 验证语法和导入正确性
- [ ] 重新构建镜像

**参考文档**：`costq/docs/20260118_CloudTrail_MCP问题/20250119_loguru_迁移到_logging.md`

---

## ✅ 改造验证

### 1. 单元测试
```python
# tests/test_account_context.py
import pytest
from awslabs.your_mcp_server.server import _setup_account_context
from awslabs.your_mcp_server.server import AccountNotFoundError

@pytest.mark.asyncio
async def test_setup_account_context_aksk():
    """测试 AKSK 账号凭证设置"""
    result = await _setup_account_context("test-aksk-account-id")
    assert result["auth_type"] == "aksk"
    assert result["account_id"] is not None

@pytest.mark.asyncio
async def test_setup_account_context_iam_role():
    """测试 IAM Role 账号凭证设置"""
    result = await _setup_account_context("test-role-account-id")
    assert result["auth_type"] == "iam_role"
    assert result["account_id"] is not None

@pytest.mark.asyncio
async def test_setup_account_context_not_found():
    """测试账号不存在异常"""
    with pytest.raises(AccountNotFoundError):
        await _setup_account_context("non-existent-account")
```

### 2. 本地测试
```bash
# 启动 MCP Server
python -m awslabs.your_mcp_server.server

# 预期输出
INFO:     Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)
```

### 3. 手动验证清单
- [ ] AKSK 账号凭证正常
- [ ] IAM Role 账号凭证正常
- [ ] External ID 正确传递
- [ ] Session Token 处理正常
- [ ] 多账号切换正常
- [ ] 默认凭证（无 target_account_id）正常
- [ ] 所有异常处理正常
- [ ] 日志不包含敏感信息

---

## 🐳 Docker 镜像构建和部署

### Step 5: 创建 Dockerfile-AgentCore-Runtime

#### 5.1 直接复制 Dockerfile

```bash
cd src/<mcp-server-name>/

# ✅ 直接复制
cp ../cloudtrail-mcp-server/Dockerfile-AgentCore-Runtime ./
```

#### 5.2 修改启动命令

**修改 Dockerfile 最后一行的 CMD**：

```dockerfile
# 原始（cloudtrail）
CMD ["opentelemetry-instrument", "python", "-m", "awslabs.cloudtrail_mcp_server.server"]

# 修改为你的包名
CMD ["opentelemetry-instrument", "python", "-m", "awslabs.your_mcp_server.server"]
```

**快速修改脚本**：
```bash
# 自动修改 CMD（macOS）
sed -i '' 's/awslabs\.cloudtrail_mcp_server/awslabs.your_mcp_server/' Dockerfile-AgentCore-Runtime

# 验证修改
grep "CMD" Dockerfile-AgentCore-Runtime
```

---

#### 5.3 Dockerfile 关键内容

**已包含的重要配置**（无需修改）：

1. **复制凭证服务**：
   ```dockerfile
   COPY --chown=app:app ./cred_extract_services /app/cred_extract_services
   ```

2. **安装额外依赖**：
   ```dockerfile
   RUN pip install --no-cache-dir uv && \
       uv pip install --python /app/.venv/bin/python \
       aws-opentelemetry-distro==0.12.2 \
       sqlalchemy>=2.0.0 \
       psycopg2-binary>=2.9.0 \
       cryptography>=41.0.0
   ```

3. **暴露端口**：
   ```dockerfile
   EXPOSE 9000
   EXPOSE 8000
   EXPOSE 8080
   ```

4. **环境变量**：
   ```dockerfile
   ENV UV_SYSTEM_PYTHON=1 \
       UV_NO_PROGRESS=1 \
       DOCKER_CONTAINER=1 \
       AWS_REGION=ap-northeast-1 \
       AWS_DEFAULT_REGION=ap-northeast-1
   ```

---

### Step 6: 构建和推送镜像

#### 6.1 使用通用部署脚本（推荐⭐）

```bash
# 从项目根目录执行
bash costq/scripts/build_and_push_template.sh <mcp-server-name>

# 示例
bash costq/scripts/build_and_push_template.sh your-mcp-server
```

**脚本会自动**：
1. ✅ 登录 ECR
2. ✅ 构建 ARM64 镜像
3. ✅ 打标签（`latest` + 时间戳）
4. ✅ 推送到 ECR

---

#### 6.2 手动构建（可选）

**如果不使用脚本，可以手动执行**：

```bash
cd src/<mcp-server-name>/

# 1. 登录 ECR
aws ecr get-login-password --region ap-northeast-1 --profile 3532 | \
  docker login --username AWS --password-stdin <account-id>.dkr.ecr.ap-northeast-1.amazonaws.com

# 2. 构建镜像
docker buildx build \
  --platform linux/arm64 \
  -f Dockerfile-AgentCore-Runtime \
  -t <ecr-url>/<mcp-server-name>:latest \
  --load \
  .

# 3. 推送镜像
docker push <ecr-url>/<mcp-server-name>:latest
```

---

### Step 7: 更新 Runtime 和验证

#### 7.1 更新 Runtime
```bash
# 使用 AWS CLI 更新
aws bedrock-agentcore-control update-runtime \
  --profile 3532 \
  --region ap-northeast-1 \
  --runtime-identifier <runtime-id> \
  --container-image <ecr-url>/<mcp-server-name>:latest
```

#### 7.2 刷新 Gateway
⚠️ **重要**：按照 DEEPV.md 说明，需要手动刷新 Gateway

#### 7.3 验证部署

```bash
# 查看日志
kubectl logs -f -n costq-fastapi deployment/costq-fastapi

# 验证健康检查
curl http://<gateway-url>/health

# 测试 API
curl -X POST http://<gateway-url>/mcp \
  -H "Content-Type: application/json" \
  -d '{"method": "tools/list"}'
```

---

## ⚠️ Step 8: 检查复杂参数类型兼容性

### 8.1 背景：filter_expression 参数类型问题

**在改造过程中,必须检查是否有复杂参数类型（如 `dict`、嵌套对象等）,它们可能触发 FastMCP/Gateway/Runtime 的序列化兼容性问题。**

#### 8.1.1 问题症状

**典型错误**：
```
Error executing tool xxx: 1 validation error for function_name
Arguments\nparameter_name\n
Input should be a valid string [type=string_type, input_value={'key': 'value'}, input_type=dict]
```

**调用链**：
```
模型
  ↓
AgentCore Gateway (可能序列化 dict → JSON string)
  ↓
AgentCore Runtime
  ↓
MCP Server FastMCP (Pydantic 验证)
  ↓
Tool Function
```

---

#### 8.1.2 问题根源

**三层问题叠加**：

1. **FastMCP Schema 生成缺陷**
   - `dict` 类型无法生成正确的 `type: object` Schema
   - 导致 OpenAPI Schema 不完整或错误

2. **Gateway 序列化行为不确定**
   - Schema 说 `type: object` → Gateway 序列化为 JSON string ✅
   - Schema 说 `type: string` → Gateway 不序列化,传递原始类型 ❌
   - Schema 缺少 `type` → Gateway 行为未定义 ❌

3. **Pydantic 验证拦截**
   - 函数签名定义为 `str`,但接收到 `dict` → ValidationError ❌
   - Pydantic 在函数调用前验证,函数体内的转换逻辑无法执行

---

#### 8.1.3 检查方法

```bash
# 查找所有使用 dict 类型参数的函数
cd src/<mcp-server-name>/
grep -r "Optional\[dict\]" awslabs/ --include="*.py"
grep -r ": dict" awslabs/ --include="*.py"
grep -r "Dict\[" awslabs/ --include="*.py"
```

**需要关注的参数类型**：
- ✅ `Optional[dict]` - **高风险**
- ✅ `dict[str, Any]` - **高风险**
- ✅ `List[dict]` - **高风险**
- ⚠️ `Optional[str]` - 如果实际传递 JSON 对象 - **中风险**
- ⚠️ 嵌套 Pydantic Model - **中风险**

---

#### 8.1.4 解决方案：Union 类型 + 智能转换

**推荐模式**：同时支持 `str` 和 `dict` 输入

```python
from typing import Union
import json

# 1. 定义智能转换函数（可复用）
def parse_complex_param(
    param: Optional[Union[str, dict]],
    function_name: str,
    param_name: str = "complex_param"
) -> Optional[dict]:
    """解析复杂参数,支持 JSON 字符串和 dict 对象.

    Args:
        param: JSON 字符串或 dict 对象
        function_name: 调用此函数的函数名（用于日志）
        param_name: 参数名（用于日志）

    Returns:
        解析后的 dict 或 None

    Raises:
        ValueError: JSON 格式无效
    """
    if not param:
        return None

    # 🔍 调试日志: 记录接收到的类型
    logger.info(
        "🔍 [%s] %s type: %s, value: %s",
        function_name,
        param_name,
        type(param).__name__,
        str(param)[:200]
    )

    # ⚡ 智能处理: 如果是 dict,直接使用
    if isinstance(param, dict):
        logger.warning(
            "⚠️ [%s] Received dict for %s instead of string! Auto-converting...",
            function_name,
            param_name
        )
        return param

    # ✅ 标准处理: JSON 字符串解析
    try:
        parsed = json.loads(param)
        logger.info("✅ [%s] Successfully parsed %s", function_name, param_name)
        return parsed
    except json.JSONDecodeError as e:
        logger.error(
            "❌ [%s] Invalid JSON format for %s: %s",
            function_name,
            param_name,
            str(e)
        )
        raise ValueError(f"Invalid JSON format for {param_name}: {e}")


# 2. 在 tool 函数中使用
@server.tool(name="example-tool", description="...")
async def example_tool(
    ctx: Context,
    filter_expression: Annotated[
        Optional[Union[str, dict]],  # ✅ 同时支持 str 和 dict
        Field(
            description=(
                "Filter expression as a JSON string or dict object. "
                "Example: '{\"Dimensions\": {\"Key\": \"SERVICE\", \"Values\": [\"EC2\"]}}'"
            )
        ),
    ] = None,
    tags: Annotated[
        Optional[Union[str, List[dict]]],  # ✅ 复杂嵌套类型也适用
        Field(description="Tags as JSON string or list of dicts")
    ] = None,
    target_account_id: Optional[str] = None,
) -> dict[str, Any]:
    try:
        # 账号上下文初始化
        if target_account_id:
            await _setup_account_context(target_account_id)

        # ✅ 智能解析复杂参数
        filter_dict = parse_complex_param(
            filter_expression,
            "example_tool",
            "filter_expression"
        )

        # 解析 tags (如果有)
        tags_list = None
        if tags:
            if isinstance(tags, list):
                tags_list = tags
            else:
                tags_list = json.loads(tags)

        # 业务逻辑
        client = create_aws_client("service", "region")
        response = client.some_api_call(
            Filter=filter_dict,
            Tags=tags_list
        )
        return format_response("success", response)

    except Exception as e:
        return await handle_aws_error(ctx, e, "operation", "Service")
```

---

#### 8.1.5 改造检查清单

**在改造任何 MCP Server 时,必须完成以下检查**：

- [ ] 查找所有 `dict` 类型参数
- [ ] 查找所有嵌套对象参数（`List[dict]` 等）
- [ ] 查找所有可能接收 JSON 的 `str` 参数
- [ ] 对于每个复杂参数:
  - [ ] 参数类型改为 `Union[str, dict]` (或相应的 Union 类型)
  - [ ] 添加参数描述: "JSON string or dict object"
  - [ ] 添加智能解析逻辑
  - [ ] 添加调试日志（记录接收到的类型）
  - [ ] 添加单元测试（测试两种输入格式）

---

#### 8.1.6 示例：完整的改造对比

**改造前（可能出错）**：
```python
@server.tool(name="get-resources", description="...")
async def get_resources(
    ctx: Context,
    filter_expression: Annotated[
        Optional[dict],  # ❌ 可能触发 Schema 生成问题
        Field(description="Filter expression")
    ] = None,
) -> dict[str, Any]:
    client = create_aws_client("service", "region")

    # ❌ 如果 Gateway 传递的是 JSON string,这里会失败
    response = client.list_resources(Filter=filter_expression)
    return format_response("success", response)
```

**改造后（兼容两种类型）**：
```python
@server.tool(name="get-resources", description="...")
async def get_resources(
    ctx: Context,
    filter_expression: Annotated[
        Optional[Union[str, dict]],  # ✅ 同时支持
        Field(
            description=(
                "Filter expression as a JSON string or dict object. "
                "Example: '{\"Key\": \"Value\"}'"
            )
        ),
    ] = None,
    target_account_id: Optional[str] = None,
) -> dict[str, Any]:
    try:
        # 账号上下文初始化
        if target_account_id:
            await _setup_account_context(target_account_id)

        # ✅ 智能解析
        filter_dict = parse_complex_param(
            filter_expression,
            "get_resources",
            "filter_expression"
        )

        client = create_aws_client("service", "region")
        response = client.list_resources(Filter=filter_dict)
        return format_response("success", response)

    except ValueError as e:
        return format_response('error', {'error_type': 'invalid_json'}, str(e))
    except Exception as e:
        return await handle_aws_error(ctx, e, "list_resources", "Service")
```

---

#### 8.1.7 测试验证

**单元测试（必须包含两种输入格式）**：
```python
import pytest
from awslabs.your_mcp_server.tools import parse_complex_param

def test_parse_complex_param_dict():
    """测试 dict 输入"""
    result = parse_complex_param(
        {"Key": "Value"},
        "test_func",
        "filter"
    )
    assert result == {"Key": "Value"}

def test_parse_complex_param_json_string():
    """测试 JSON 字符串输入"""
    result = parse_complex_param(
        '{"Key": "Value"}',
        "test_func",
        "filter"
    )
    assert result == {"Key": "Value"}

def test_parse_complex_param_none():
    """测试 None 输入"""
    result = parse_complex_param(None, "test_func", "filter")
    assert result is None

def test_parse_complex_param_invalid_json():
    """测试无效 JSON"""
    with pytest.raises(ValueError, match="Invalid JSON format"):
        parse_complex_param(
            "invalid json",
            "test_func",
            "filter"
        )
```

---

#### 8.1.8 相关文档

**详细分析**：
- `costq/docs/20260120_filter_expression分析/问题总结与修复方案.md`
- `costq/docs/20260120_filter_expression分析/step6_根本原因分析.md`

**参考代码**：
- `src/costq-risp-mcp-server/handlers/sp_handler.py` (已修复)
- `src/costq-risp-mcp-server/handlers/ri_handler.py` (已修复)

---

## 🐛 常见问题排查

### 问题 1: ModuleNotFoundError: No module named 'cred_extract_services'
**原因**：未正确复制 cred_extract_services 目录

**解决**：
```bash
# 确认目录存在
ls -la cred_extract_services/

# 重新复制
cp -r ../cloudtrail-mcp-server/cred_extract_services ./
```

### 问题 2: DatabaseConnectionError
**原因**：数据库配置错误

**解决**：
```bash
# 检查环境变量
echo $DATABASE_URL
echo $RDS_SECRET_NAME

# 测试数据库连接
python -c "
from cred_extract_services.database import query_account
import asyncio
result = asyncio.run(query_account('test-account-id'))
print(result)
"
```

### 问题 3: CredentialDecryptionError
**原因**：加密密钥错误

**解决**：
```bash
# 检查加密密钥
echo $ENCRYPTION_KEY | base64 -d | xxd  # 应该是 32 字节

# 重新生成密钥（仅测试环境）
python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
```

### 问题 4: AssumeRoleError
**原因**：IAM Role 配置错误

**解决**：
```bash
# 检查 Role ARN 和 External ID
python -c "
from cred_extract_services.database import query_account
import asyncio
account = asyncio.run(query_account('test-role-account-id'))
print(f'Role ARN: {account[\"role_arn\"]}')
print(f'External ID: {account[\"external_id\"]}')
"

# 测试 AssumeRole
aws sts assume-role \
  --role-arn <role-arn> \
  --role-session-name test \
  --external-id <external-id>
```

### 问题 5: 启动后无法连接（127.0.0.1:8000）
**原因**：FastMCP 未配置 `host="0.0.0.0"`

**解决**：
```python
# 检查 server.py
mcp = FastMCP(
    name='awslabs.your-mcp-server',
    host="0.0.0.0",  # ✅ 必须！
    stateless_http=True
)
```

---

## 📝 改造检查清单

### 代码修改
- [ ] 复制 `cred_extract_services/` 目录
- [ ] 修改 `server.py` FastMCP 配置（添加 `host` 和 `stateless_http`）
- [ ] 添加 `_setup_account_context()` 函数到 `server.py`
- [ ] 修改所有 tool 函数（添加 `target_account_id` 参数）
- [ ] 添加账号上下文初始化代码
- [ ] 添加异常处理
- [ ] 更新 tool 描述文档

### 测试验证
- [ ] 单元测试通过
- [ ] 本地启动测试通过
- [ ] 手动验证通过
- [ ] 异常处理验证通过

### Docker 部署
- [ ] 复制 `Dockerfile-AgentCore-Runtime`
- [ ] 修改 CMD 启动命令
- [ ] 构建镜像成功
- [ ] 推送到 ECR 成功
- [ ] 更新 Runtime 成功
- [ ] 刷新 Gateway

### 文档更新
- [ ] README 更新
- [ ] API 文档更新
- [ ] 使用示例更新

---

## 🎉 改造完成

恭喜！你已经完成了 MCP Server 的多账号改造。

**下一步**：
1. 提交代码到 Git
2. 更新项目文档
3. 通知团队成员新功能

**参考资料**：
- [改造需求分析](./01_改造需求分析.md)
- [CloudTrail MCP Server 源码](../../src/cloudtrail-mcp-server/)
- [日志迁移指南](../20260118_CloudTrail_MCP问题/20250119_loguru_迁移到_logging.md)