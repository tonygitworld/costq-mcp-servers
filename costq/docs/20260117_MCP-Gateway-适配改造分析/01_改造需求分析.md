...
**⚠️ 重要：直接在 server.py 中添加，不要创建独立模块**

**为什么放在 server.py 中？**
1. ✅ **符合 FastMCP 架构规范** - server.py 是公共函数的标准位置
2. ✅ **导入路径更简单** - Tools 直接从 `server` 模块导入
3. ✅ **100% 可复用** - 从 CloudTrail MCP 复制后完全不需要修改
4. ❌ **避免过度设计** - 不需要为一个函数创建单独的模块

**位置：在 FastMCP 初始化之后、main() 之前**
...
**在改造和维护 MCP Server 时,必须遵循以下日志级别使用标准**

#### 10.1.1 日志级别定义

| 级别 | 使用场景 | 生产环境 | 示例 |
|------|---------|---------|------|
| **DEBUG** | 详细的诊断信息,仅用于开发和调试 | ❌ 默认禁用 | 函数参数、中间变量值、详细执行流程 |
| **INFO** | 关键业务流程节点,正常操作确认 | ✅ 默认启用 | API 调用开始/完成、数据库查询成功、凭证设置完成 |
| **WARNING** | 非预期但可处理的情况,不影响核心功能 | ✅ 默认启用 | 接收到 dict 而非 string (自动转换)、使用默认值、兼容性处理 |
| **ERROR** | 错误情况,影响当前操作但不影响整体服务 | ✅ 默认启用 | API 调用失败、数据库连接失败、参数验证失败 |
| **CRITICAL** | 严重错误,可能导致服务不可用 | ✅ 默认启用 | 系统初始化失败、配置缺失、资源耗尽 |
...
```python
# ✅ 正确使用 DEBUG - 详细的内部状态
logger.debug("🔍 接收参数: account_id=%s, region=%s", account_id, region)
logger.debug("查询条件: %s", query_params)
logger.debug("中间结果: %s", intermediate_data)

# ❌ 错误使用 DEBUG - 这应该用 INFO
logger.debug("✅ 凭证设置完成")  # 改为 INFO
logger.debug("开始查询 CloudTrail 事件")  # 改为 INFO
```
...
logger.info("查询 CloudTrail 事件: %d 条", len(events))
logger.info("数据库连接成功")

# ❌ 错误使用 INFO - 过于详细的内部状态
logger.info("循环第 %d 次", i)  # 改为 DEBUG
logger.info("变量 x 的值: %s", x)  # 改为 DEBUG
```

**3. WARNING 级别原则**
```python
# ✅ 正确使用 WARNING - 非预期但可处理
logger.warning("⚠️ 接收到 dict 类型参数,应为 string,自动转换中...")
logger.warning("未提供 region 参数,使用默认值: %s", default_region)
logger.warning("凭证即将过期,建议刷新")

# ❌ 错误使用 WARNING - 正常的业务逻辑
logger.warning("未提供 target_account_id,使用默认凭证")  # 改为 INFO
logger.warning("过滤条件为空")  # 改为 INFO 或 DEBUG

# ❌ 错误使用 WARNING - 这应该是 ERROR
logger.warning("数据库连接失败")  # 改为 ERROR
logger.warning("API 调用失败")  # 改为 ERROR
```
...
logger.error("❌ API 调用失败: %s", error_message)
logger.error("❌ 参数验证失败: %s", validation_error)
logger.error("❌ 凭证解密失败", exc_info=True)

# ❌ 错误使用 ERROR - 非预期但已处理的情况
logger.error("接收到 dict 类型,转换为 string")  # 改为 WARNING
```

**5. CRITICAL 级别原则**
```python
# ✅ 正确使用 CRITICAL - 系统级别严重错误
logger.critical("🔥 环境变量 ENCRYPTION_KEY 未配置,服务无法启动")
logger.critical("🔥 数据库连接池耗尽,服务不可用")
logger.critical("🔥 内存不足,OOM 即将发生")

# ❌ 错误使用 CRITICAL - 业务操作失败
logger.critical("API 调用失败")  # 改为 ERROR
logger.critical("账号不存在")  # 改为 ERROR
```
...
**生产环境必须配置日志级别为 INFO 或更高**

```python
# server.py 或 __init__.py
import logging

# ✅ 生产环境配置
logging.basicConfig(
    level=logging.INFO,  # 默认 INFO
...
**为什么不在生产环境启用 DEBUG？**
- ❌ **性能影响**：大量日志输出会显著降低性能
- ❌ **存储成本**：CloudWatch 日志费用增加
- ❌ **信息过载**：关键信息被淹没在大量调试信息中
- ❌ **安全风险**：DEBUG 日志可能包含敏感信息
...
aws bedrock-agentcore-control update-runtime \
  ...
  --environment-variables LOG_LEVEL=DEBUG  # 临时调试
```
...
- ❌ 数据库密码
- ❌ 加密密钥 (ENCRYPTION_KEY)
- ❌ External ID（IAM Role）
- ❌ 用户个人身份信息 (PII)

**错误示例**：
```python
# ❌ 严重安全问题！
logger.info(f"使用凭证: {credentials['secret_access_key']}")
logger.debug(f"数据库密码: {db_password}")
logger.info(f"加密密钥: {encryption_key}")
```
...
# ✅ 脱敏处理
logger.debug(f"Access Key ID: {access_key_id[:4]}****{access_key_id[-4:]}")
```
...
try:
    ...
    logger.error("❌ 凭证解密失败", exc_info=True)  # exc_info 包含堆栈,但不包含变量值
...
try:
    ...
    logger.error(f"解密失败: {e}, 数据: {encrypted_data}")  # 可能暴露加密数据
```
...
# ❌ 不推荐 - 非结构化字符串
logger.info(f"API 调用成功: account={account_id}, region={region}, op=DescribeInstances, duration={duration}ms")
```

**优势**：
- ✅ CloudWatch Insights 更容易查询
- ✅ 可以按字段过滤和统计
- ✅ 格式统一,便于解析
...
# ❌ 不好的日志 - 缺少上下文
logger.info(f"查询完成,共 {len(events)} 条")
```
...
```python
# ❌ 错误 - 在循环中大量日志
for item in large_list:  # 1000+ 条
    logger.info(f"处理项目: {item}")  # 产生 1000+ 条日志！

# ✅ 正确 - 汇总日志
logger.info(f"开始处理 {len(large_list)} 个项目")
for item in large_list:
    logger.debug(f"处理项目: {item}")  # DEBUG 级别,生产环境不输出
logger.info(f"✅ 处理完成,共 {len(large_list)} 个项目")
```
...
```bash
# 检查所有 logger.debug() 调用
grep -r "logger\.debug(" awslabs/ --include="*.py" -A 1

# 检查所有 logger.info() 调用
grep -r "logger\.info(" awslabs/ --include="*.py" -A 1

# 检查所有 logger.warning() 调用
grep -r "logger\.warning(" awslabs/ --include="*.py" -A 1

# 检查所有 logger.error() 调用
grep -r "logger\.error(" awslabs/ --include="*.py" -A 1

# 检查可能的敏感信息泄露
grep -r "secret\|password\|token\|key" awslabs/ --include="*.py" | grep "logger\."
```
...
**日志修复记录**：
- 见最近的日志级别修复提交
- 参考 `billing-cost-management-mcp-server` 的日志修复
