...
class EmailVerificationService:
    ...
    async def send_verification_code(
        ...
        logger.info(f"📝 生成验证码 - email: {email}, code: {code}, expires_at: {expires_at}")
        ...
        if not email_result.get("success"):
            logger.error(f"❌ 验证码邮件发送失败: {email_result.get('error')}")
            return {
                "success": False,
    ...
    async def send_activation_email(
        ...
        if not email_result.get("success"):
            logger.error(f"❌ 激活邮件发送失败: {email_result.get('error')}")
            return {
                "success": False,
    ...
    def mark_activation_used(self, activation_id: str):
        ...
        if activation:
            ...
            self.db.commit()
            logger.info(f"✅ 激活Token已使用 - activation_id: {activation_id}")
