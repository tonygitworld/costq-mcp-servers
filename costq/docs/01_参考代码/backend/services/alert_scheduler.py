...
class AlertScheduler:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ 无效的时区 '{self.timezone_str}'，回退到 UTC")
        ...
        self.alert_agent: AlertAgentManager | None = None
        ...
        logger.info(f"   - 最大并发: {self.max_concurrent_alerts}")
        logger.info(f"   - 重试次数: {self.max_retries}")
        logger.info(f"   - 检查间隔: {self.check_interval_hours}小时")
    ...
    def start(self) -> None:
        ...
        try:
            # 初始化 AlertAgentManager（延迟初始化，避免循环依赖）
            if self.alert_agent is None:
                self.alert_agent = AlertAgentManager()
            ...
            logger.error(f"❌ AlertScheduler 启动失败: {e}", exc_info=True)
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            logger.error(f"❌ AlertScheduler 停止失败: {e}", exc_info=True)
    ...
    def _job_executed_listener(self, event) -> None:
        ...
        if event.exception:
            ...
            logger.info(f"✅ 调度任务执行成功: {event.job_id}")
    ...
    async def scan_and_execute_alerts(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"📊 查询到 {total_alerts} 个启用的告警")
            ...
            logger.info(f"   总告警数: {total_alerts}")
            logger.info(f"   已执行: {executed_count}")
            ...
            logger.error(f"❌ 扫描和执行告警失败: {e}", exc_info=True)
            raise
        finally:
            db.close()
    ...
    def _should_execute_alert(
        ...
        if alert.last_checked_at is None:
            # 新创建的告警，立即执行
            logger.debug(f"🆕 新告警 (从未执行): {alert.display_name}")
            return True
    ...
    def _update_alert_status_sync(self, alert_id: str, result: dict[str, Any]) -> None:
        ...
        - 执行日志已由 alert_agent.execute_alert_check() 内部创建和更新
        - 避免重复创建日志记录（修复 #2）
        """
        db = next(get_db())
        try:
            ...
            if not alert:
                logger.warning(f"⚠️ 更新状态失败: 找不到告警 {alert_id}")
                return
            ...
            logger.debug(f"✅ 已更新 last_checked_at: Alert {str(alert_id)[:8]}...")

        except Exception:
            logger.error(f"❌ 更新告警状态失败: Alert ID {alert_id}", exc_info=True)
        finally:
            db.close()
    ...
    def _log_failure_sync(self, alert_id: str, error_message: str) -> None:
        ...
        try:
            ...
            if not alert:
                logger.warning(f"⚠️ 无法记录失败日志: 找不到告警 {alert_id}")
                return
            ...
            db.add(log)
            db.commit()
            logger.info(f"📝 已记录失败日志: Alert ID {alert_id}")
        except Exception:
            logger.error(f"❌ 记录失败日志时出错: Alert ID {alert_id}", exc_info=True)
        finally:
            db.close()
    ...
    async def _execute_single_alert(self, alert: MonitoringConfig) -> dict[str, Any]:
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    # 确保 AlertAgentManager 已初始化
                    if self.alert_agent is None:
                        self.alert_agent = AlertAgentManager()

                    # 调用 AlertAgentManager 执行检查 (IO密集型，本身就是 async)
                    result = await self.alert_agent.execute_alert_check(
                        alert_id=alert.id,
