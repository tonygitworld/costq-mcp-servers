...
class SecretsManager:
    ...
    def __init__(self, region_name: str = "us-east-1"):
        ...
        try:
            ...
            logger.info(f"✅ Secrets Manager 客户端初始化成功 - Region: {region_name}")
    ...
    def get_secret(self, secret_name: str, version_stage: str = "AWSCURRENT") -> dict:
        ...
        """
        try:
            logger.debug(f"🔍 获取密钥: {secret_name} (Stage: {version_stage})")
            ...
            logger.error(f"❌ 获取密钥失败: {secret_name}, Error: {e}")
            raise
    ...
    def get_database_credentials(
        ...
        logger.info(f"🔐 获取数据库凭证 - Environment: {environment}, App: {app_name}")
    ...
    def refresh_secret(self, secret_name: str):
        ...
        """
        logger.info(f"🔄 刷新密钥缓存: {secret_name}")
    ...
    def list_secrets(self, filters: dict | None = None) -> list:
        ...
        try:
            ...
            logger.info(f"📋 找到 {len(secrets)} 个密钥")
            return secrets
