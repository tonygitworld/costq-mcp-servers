...
def get_database_url() -> str:
    ...
    try:
        ...
        if env_db_url:
            ...
            logger.info(f"✅ 使用环境变量配置的数据库: PostgreSQL, Host: {safe_url.split('/')[0]}")
            return env_db_url
        ...
        logger.info(
            f"✅ 从Secrets Manager获取数据库配置: PostgreSQL, Host: {safe_url.split('/')[0]}"
        )

        return database_url
    except Exception as e:
        logger.error(f"❌ 获取数据库连接字符串失败: {e}")
        raise
...
def _init_engine():
    ...
    logger.info(f"✅ 数据库引擎创建成功 - Environment: {settings.ENVIRONMENT}")
