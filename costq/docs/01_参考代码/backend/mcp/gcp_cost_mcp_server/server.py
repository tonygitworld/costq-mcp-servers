...
# Get account ID from environment (set by dynamic_clients.py)
DEFAULT_ACCOUNT_ID = os.getenv("GCP_ACCOUNT_ID")
ENCRYPTION_KEY = os.getenv("ENCRYPTION_KEY")

# logger.info(f"🔍 MCP Server Environment Check:")  # 已静默 - 每次 MCP 启动都重复
# logger.info(f"  - GCP_ACCOUNT_ID: {DEFAULT_ACCOUNT_ID or '(not set)'}")
# logger.info(f"  - ENCRYPTION_KEY: {'✅ Set' if ENCRYPTION_KEY else '❌ Not set'}")
# logger.info(f"  - PROJECT_ID: {os.getenv('GCP_PROJECT_ID', '(not set)')}")

# if DEFAULT_ACCOUNT_ID:
#     logger.info(f"🔑 MCP Server using GCP Account ID: {DEFAULT_ACCOUNT_ID}")
# else:
if not DEFAULT_ACCOUNT_ID:
    logger.warning("⚠️  GCP_ACCOUNT_ID not set - tools will not work!")

# Import handlers
from backend.mcp.gcp_cost_mcp_server.handlers.billing_handler import (
    get_cost_by_label,
    get_cost_by_project,
    get_cost_by_service,
    get_cost_by_sku,
...
from backend.mcp.gcp_cost_mcp_server.handlers.budget_handler import (
    create_budget,
    get_budget_status,
    list_budgets,
)
...
from backend.mcp.gcp_cost_mcp_server.handlers.cud_handler_advanced import (
    get_cud_resource_usage,
    get_cud_status_check,
)
from backend.mcp.gcp_cost_mcp_server.handlers.cud_handler_comparison import (
...
mcp = FastMCP("GCP Cost Management")
...
async def gcp_cost_by_service(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CostByServiceParams
...
async def gcp_cost_by_project(
    ...
    **Supports organization-level filtering via billing_account_id.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CostByProjectParams
...
async def gcp_daily_cost_trend(
    ...
    from backend.mcp.gcp_cost_mcp_server.models import DailyCostTrendParams
...
async def gcp_cost_by_label(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CostByLabelParams
...
async def gcp_cost_by_sku(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CostBySkuParams
...
async def gcp_cost_summary(
    ...
    **Supports both project-level and organization-level summaries.**
    ...
    try:
        ...
        from backend.mcp.gcp_cost_mcp_server.models import CostSummaryParams
...
async def gcp_vm_rightsizing_recommendations(
    ...
    from backend.mcp.gcp_cost_mcp_server.models import VmRightsizingRecommendationsParams
...
async def gcp_idle_resources(
    ...
    from backend.mcp.gcp_cost_mcp_server.models import IdleResourcesParams
...
async def gcp_commitment_recommendations(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    result = await get_commitment_recommendations(
        None, project_id, billing_account_id, location, DEFAULT_ACCOUNT_ID
    )
    return json.dumps(result, ensure_ascii=False, default=str)
...
async def gcp_mark_recommendation_status(
    ...
    return await mark_recommendation_status(
        None, recommendation_name, state, state_metadata, DEFAULT_ACCOUNT_ID
    )
...
async def gcp_list_budgets(
    ...
    """
    return await list_budgets(None, billing_account_id, DEFAULT_ACCOUNT_ID)
...
async def gcp_get_budget_status(
    ...
    result = await get_budget_status(None, budget_name, DEFAULT_ACCOUNT_ID)
    return json.dumps(result, ensure_ascii=False, default=str)
...
async def gcp_create_budget(
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CreateBudgetParams
...
async def gcp_list_commitments(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import ListCommitmentsParams
...
async def gcp_cud_coverage(
    ...
    **Important**: Data excludes last 2 days due to billing lag.
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CudCoverageParams
...
async def gcp_cud_savings_analysis(
    ...
    **Supports both project-level and organization-level queries.**
    ...
    from backend.mcp.gcp_cost_mcp_server.models import CudSavingsAnalysisParams
...
async def gcp_cud_vs_ondemand_comparison(
    ...
    **Supports organization-level analysis** via billing_account_id.
    ...
    result = await get_cud_vs_ondemand_comparison(
        None, project_id, billing_account_id, start_date, end_date, scenario, DEFAULT_ACCOUNT_ID
    )
...
async def gcp_flexible_cud_analysis(
    ...
    **Supports organization-level analysis** via billing_account_id.
    ...
    result = await get_flexible_cud_analysis(
        None, project_id, billing_account_id, start_date, end_date, granularity, DEFAULT_ACCOUNT_ID
    )
...
def main():
    """Main entry point for the GCP Cost Management MCP server"""
    # logger.info("GCP Cost Management MCP Server starting - 18 tools available")  # 已静默 - 每次启动都重复
