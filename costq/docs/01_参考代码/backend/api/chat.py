...
async def create_chat_session(
    ...
    try:
        session = chat_storage.create_session(
            user_id=user_id, org_id=org_id, title=session_create.title
        )
        return ChatSessionResponse(**session)

    except Exception as e:
        logger.error(f"❌ 创建会话失败 - User: {user_id}, Error: {e}")
        raise HTTPException(
...
async def get_chat_session(
    ...
    if session["user_id"] != user_id:
        logger.warning(f"⚠️  权限不足 - User: {user_id} 尝试访问 Session: {session_id}")
...
async def update_chat_session(
    ...
    if session["user_id"] != user_id:
        logger.warning(f"⚠️  权限不足 - User: {user_id} 尝试更新 Session: {session_id}")
    ...
    try:
        ...
        logger.error(f"❌ 更新会话失败 - Session: {session_id}, Error: {e}")
        raise HTTPException(
...
async def delete_chat_session(
    ...
    if session["user_id"] != user_id:
        logger.warning(f"⚠️  权限不足 - User: {user_id} 尝试删除 Session: {session_id}")
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="无权删除该会话")

    try:
        chat_storage.delete_session(session_id)
        logger.info(f"✅ 会话已删除 - Session: {session_id}, User: {user_id}")

    except Exception as e:
        logger.error(f"❌ 删除会话失败 - Session: {session_id}, Error: {e}")
        raise HTTPException(
...
async def create_chat_message(
    ...
    try:
        ...
        logger.error(f"❌ 保存消息失败 - Session: {session_id}, Error: {e}")
        raise HTTPException(
...
async def delete_chat_message(
    ...
    try:
        ...
        logger.info(f"✅ 消息已删除 - Message: {message_id}, User: {user_id}")

    except Exception as e:
        logger.error(f"❌ 删除消息失败 - Message: {message_id}, Error: {e}")
        raise HTTPException(
