...
async def create_gcp_account(
    ...
    logger.info(f"📝 创建 GCP 账号 - Name: {account_create.account_name}")
    ...
    if not validation["valid"]:
        logger.error(f"❌ 凭证验证失败: {validation['error']}")
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"✅ GCP 账号创建成功 - ID: {gcp_account.id}")
    ...
    if (
        ...
        try:
            ...
            if extracted_billing_id:
                ...
                logger.info(f"✅ 已自动提取并保存 billing_account_id: {extracted_billing_id}")
            else:
                logger.warning("⚠️ 未能从 BigQuery 提取 billing_account_id")
        except Exception as e:
            # 不影响账号创建流程，只记录警告
            logger.warning(f"⚠️ 自动提取 billing_account_id 失败: {e}")
...
async def list_gcp_accounts(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == "admin":
        # 管理员可以看到本组织所有账号
        logger.info(f"✅ 管理员访问，返回本组织全部 {len(org_accounts)} 个GCP账号")
    ...
    logger.info(f"✅ 返回 {len(response)} 个 GCP 账号")
    return response
...
async def get_gcp_account(account_id: str, current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"🔍 获取 GCP 账号 - Org: {current_user['org_id']}, ID: {account_id}")

    account = account_storage.get_account(account_id)

    if not account:
        logger.warning(f"⚠️  GCP 账号不存在 - ID: {account_id}")
        raise HTTPException(
...
async def update_gcp_account(
    ...
    logger.info(f"📝 更新 GCP 账号 - Org: {current_user['org_id']}, ID: {account_id}")
...
async def delete_gcp_account(account_id: str, current_user: dict = Depends(get_current_admin_user)):
    ...
    logger.info(f"🗑️  删除 GCP 账号 - Org: {current_user['org_id']}, ID: {account_id}")
    ...
    logger.info(f"✅ GCP 账号删除成功 - ID: {account_id}")
...
async def validate_gcp_account_credentials(account_id: str):
    ...
    logger.info(f"🔍 验证 GCP 账号凭证 - ID: {account_id}")
    ...
    if validation["valid"]:
        logger.info(f"✅ 凭证验证成功 - Project: {validation['project_id']}")
    else:
        logger.error(f"❌ 凭证验证失败: {validation['error']}")
