...
async def get_system_template(template_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"✅ 获取模板详情成功 - ID: {template_id}")
        return row_dict
...
async def get_slash_commands(db=Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"✅ 获取斜杠命令成功 - Count: {len(commands)}")
        return commands

    except Exception as e:
        logger.error(f"❌ 获取斜杠命令失败: {e}")
        ...
        for row in rows:
            ...
            if row_dict.get("variables"):
                try:
                    ...
                    row_dict["variables"] = None
            templates.append(row_dict)
        logger.info(f"✅ 获取用户模板成功 - User: {current_user['id']}, Count: {len(templates)}")
        return templates
...
async def update_user_template(
    ...
    try:
        ...
        logger.info(f"✅ 更新用户模板成功 - User: {current_user['id']}, Template: {template_id}")
        return row_dict
...
async def delete_user_template(
    ...
    try:
        ...
        db.commit()
        logger.info(f"✅ 删除用户模板成功 - User: {current_user['id']}, Template: {template_id}")
        return None
