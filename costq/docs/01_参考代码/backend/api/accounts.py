...
async def create_account(
    ...
    logger.info(f"✅ 凭证验证成功 - Account: {validation['account_id']}, ARN: {validation['arn']}")
    ...
    try:
        ...
        logger.info(f"✅ 账号创建成功 - ID: {aws_account.id}")
...
async def list_accounts(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] == "admin":
        # 管理员可以看到本组织所有账号
        logger.info(f"✅ 管理员访问，返回本组织全部 {len(org_accounts)} 个账号")
    ...
    logger.info(f"✅ 返回 {len(response)} 个账号")
    return response
...
async def get_account(account_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if not account:
        logger.warning(f"⚠️  账号不存在 - ID: {account_id}")
        raise HTTPException(
...
async def update_account(
    ...
    logger.info(f"📝 更新账号 - Org: {current_user['org_id']}, ID: {account_id}")
...
async def delete_account(account_id: str, current_user: dict = Depends(get_current_admin_user)):
    ...
    logger.info(f"🗑️  删除账号 - Org: {current_user['org_id']}, ID: {account_id}")
...
async def validate_account_credentials(
    ...
    logger.info(f"🔍 验证账号凭证 - Org: {current_user['org_id']}, ID: {account_id}")
    ...
    if validation["valid"]:
        logger.info(f"✅ 凭证验证成功 - Account: {validation['account_id']}")
    else:
        logger.error(f"❌ 凭证验证失败: {validation['error']}")
...
async def get_external_id(current_user: dict = Depends(get_current_user)):
    ...
    logger.info(f"📋 获取 External ID - Org: {current_user['org_id']}, External ID: {external_id}")
...
async def create_iam_role_account(
    ...
    if not validation["valid"]:
        logger.error(f"❌ IAM Role 验证失败: {validation['error']}")
        raise HTTPException(
    ...
    try:
        saved_account = account_storage.create_account(aws_account)
        logger.info(f"✅ IAM Role 账号创建成功 - ID: {aws_account.id}")
