...
class AgentCoreResponseParser:
    ...
    def _add_session_id(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.session_id:
            ...
            logger.warning(f"⚠️ [Parser] 未配置 session_id，消息类型: {message.get('type')}")
        return message
    ...
    def parse_event(self, event: bytes | dict) -> list[dict[str, Any]]:
        ...
        if os.getenv("ENVIRONMENT") == "local":
            if isinstance(event, dict):
                logger.debug(f"🔍 [Parser] 收到字典事件，键: {list(event.keys())}")
            elif isinstance(event, bytes):
                logger.debug(f"🔍 [Parser] 收到bytes事件，长度: {len(event)}")
        ...
        try:
            ...
            # 如果是 bytes，解码
                ...
                logger.warning(f"未知事件类型: {type(event)}, 跳过")
                return messages
            ...
            if event_str.startswith("data: "):
                ...
                logger.debug(f"忽略非 SSE 格式事件: {event_str[:100]}")

        except Exception as e:
            logger.error(f"事件解析失败: {e}", exc_info=False)
    ...
    def _parse_data_string(self, data_str: str) -> dict[str, Any] | None:
        ...
        logger.debug(f"无法解析数据: {data_str[:100]}")
        return None
    ...
    def _convert_to_websocket_messages(self, data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if "error" in data:
            ...
            logger.error(f"Runtime 返回错误: {error_msg}")
            messages.append(
                {
                    "type": "error",
        ...
        if "event" in data:
            ...
            if event_types:
                logger.debug(f"🔍 [Parser] event字段包含: {event_types}")
                ...
                if "text" in delta:
                    ...
                    if func_result_match:
                        ...
                        try:
                            ...
                            logger.error(f"❌ 解析工具结果失败: {e}, 原始内容: {result_json[:200]}")
                        ...
                        if match:
                            ...
                            while True:
                                ...
                                if orphan_param_match:
                                    ...
                                    logger.warning(f"⚠️ 发现并合并 </invoke> 后的参数: {param_name}")
            ...
            if "contentBlockStart" in event_data:
                ...
                logger.info(f"🔍 [Parser] contentBlockStart.start键: {list(start.keys())}")
            ...
            if "contentBlockStop" in event_data:
                ...
                if matching_tool_id:
                    ...
                    try:
                        ...
                        logger.error(f"❌ 解析工具参数失败: {args_str}")
                        args = {}
            ...
            if "messageStop" in event_data:
                ...
                if stop_reason in ["end_turn", "max_tokens", "stop_sequence"]:
                    ...
                    logger.info(f"✅ 发送 complete 事件 - stopReason: {stop_reason}")
                            ...
                            if isinstance(item, dict):
                                if "json" in item:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"⚠️ 解析工具结果JSON失败: {e}, 使用原始数据")
                        ...
                        if hasattr(self, "tool_id_map"):
                            for tool_name, tid in list(self.tool_id_map.items()):
                                if tid == tool_use_id:
                                    ...
                                    logger.debug(f"🗑️  已移除工具映射: {tool_name} -> {tool_use_id}")
                                    break
