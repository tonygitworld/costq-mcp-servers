...
async def register(register_request: RegisterRequest, request: Request):
    ...
    **要求：**
    ...
    try:
        ...
        logger.info(f"🔒 租户默认禁用，等待审核 - org_id: {organization['id']}")
        ...
        if not organization.get("is_active", False):
            ...
            logger.info(f"✅ 注册成功（待审核）- user_id: {new_user['id']}, email: {register_request.email}")
            ...
            logger.debug(f"返回响应数据: {response_data}")
            return response_data

        # 租户已激活：生成并返回 Token
        logger.info(f"✅ 注册成功（已激活）- user_id: {new_user['id']}, email: {register_request.email}")
...
async def login(login_request: LoginRequest, request: Request):
    ...
    **登录流程：**
    ...
    try:
        ...
        logger.warning(f"⚠️  更新最后登录时间失败（可能数据库缺少字段）: {e}")
    ...
    return {
        ...
        "refresh_token": refresh_token,
        "token_type": "bearer",
        "expires_in": settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60,  # 24小时 = 86400秒
        "user": user_data,
        "organization": organization,
    }
...
async def activate_account(request_body: ActivateAccountRequest):
    ...
    try:
        ...
        logger.info(f"✅ 用户激活成功 - user_id: {user_id}, email: {email}")
...
async def resend_activation(request_body: ResendActivationRequest, request: Request):
    ...
    try:
        ...
        logger.error(f"❌ 重新发送激活邮件失败: {str(e)}")
        raise HTTPException(
