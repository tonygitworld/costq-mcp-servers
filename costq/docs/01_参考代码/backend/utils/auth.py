...
def create_access_token(data: dict, expires_delta: timedelta | None = None) -> str:
    ...
    ttl = (expire - datetime.utcnow()).total_seconds() / 3600  # 小时
    logger.info(
        f"🔐 Token已创建 - User: {username}, "
        f"过期时间: {expire.strftime('%Y-%m-%d %H:%M:%S')} UTC ({ttl:.1f}小时后)"
    )
    ...
    try:
        ...
        if "Signature has expired" in error_msg:
            ...
            try:
                ...
                exp_time = datetime.fromtimestamp(expired_payload.get("exp", 0))
                logger.warning(
                    f"❌ Token已过期 - User: {username}, "
                    f"过期时间: {exp_time.strftime('%Y-%m-%d %H:%M:%S')} UTC, "
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        if user_id is None:
            logger.warning("⚠️  Token中没有user_id")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="无效的认证凭证",
                headers={"WWW-Authenticate": "Bearer"},
        ...
        if user is None:
            logger.warning(f"⚠️  用户不存在 - ID: {user_id}")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="用户不存在",
                headers={"WWW-Authenticate": "Bearer"},
        ...
        if not user.get("is_active"):
            logger.warning(f"⚠️  用户已被禁用 - User: {user.get('username')}")
...
async def get_current_admin_user(current_user: dict = Depends(get_current_user)) -> dict:
    ...
    logger.debug(
        f"🔐 权限检查 - User: {current_user.get('username')}, Role: {current_user.get('role')}"
    )

    if current_user.get("role") != "admin":
        logger.warning(
            f"⚠️  权限不足 - User: {current_user.get('username')}, Role: {current_user.get('role')}"
        )
...
async def get_current_super_admin(
    ...
    logger.info(f"超级管理员验证通过 - Email: {user_email}")
    return current_user
