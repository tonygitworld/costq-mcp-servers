...
class PerformanceMetrics:
    ...
    def record_query_time(self, account_id: str, duration: float):
        ...
        try:
            ...
            with self._lock:
                ...
                # P1-2: 更新访问时间（LRU）
                self._account_access_time[account_id] = datetime.utcnow()
    ...
    def record_mcp_load_time(self, account_id: str, server_type: str, duration: float):
        ...
        try:
            ...
            logger.warning(f"⚠️ 记录MCP加载时间失败: {e}")
    ...
    def get_mcp_stats(self) -> dict:
        ...
        try:
            ...
            logger.warning(f"⚠️ 获取MCP统计失败: {e}")
            return {"error": str(e)}
    ...
    def log_summary(self):
        ...
        try:
            ...
            logger.info(f"账号数: {stats['total_accounts']}")
            logger.info(f"运行时长: {stats['uptime_seconds']:.0f}秒")
            logger.info(f"平均查询时间: {stats['avg_time']:.2f}秒")
            logger.info(f"最快查询: {stats['min_time']:.2f}秒")
            logger.info(f"最慢查询: {stats['max_time']:.2f}秒")
            logger.info(f"P50 (中位数): {stats['p50']:.2f}秒")
            logger.info(f"P90: {stats['p90']:.2f}秒")
            logger.info(f"P95: {stats['p95']:.2f}秒")
            logger.info(f"P99: {stats['p99']:.2f}秒")
            logger.info("=" * 60)
    ...
    def _cleanup_oldest_account(self):
        ...
        logger.info(f"🧹 [Metrics] 清理最旧账号的性能数据: {oldest_account} (LRU策略)")
