# MCP Server 多账号改造步骤指南

> 本文档提供**详细的、可执行的**改造步骤，适用于所有需要多账号访问能力的 MCP Server。

---

## 🎯 快速参考：哪些文件可以直接复制？

| 文件/目录 | 是否可复制 | 需要修改 | 说明 |
|-----------|-----------|---------|------|
| `cred_extract_services/` | ✅ **可直接复制** | ❌ 无需修改 | 完全通用，自包含，不依赖项目代码 |
| `Dockerfile-AgentCore-Runtime` | ✅ **可直接复制** | ❌ 无需修改 | 通用模板，使用相对路径和环境变量 |
| `entrypoint.py` | ⚠️ **需要修改** | ✅ 修改 1 处 | 需要修改导入路径（`from awslabs.<package>_mcp_server.server import mcp, setup`） |
| `costq/scripts/build_and_push_*.sh` | ⚠️ **需要修改** | ✅ 修改 1 处 | 需要修改 `MCP_SERVER_NAME` 变量 |

**总结**：
- ✅ **80% 的文件可以直接复制**（cred_extract_services/ + Dockerfile）
- ⚠️ **20% 需要简单修改**（entrypoint.py 改 1 行，部署脚本改 1 个变量）
- 🚀 **改造速度极快**：预计 10-15 分钟完成基础改造

---

## 📋 改造前准备

### 1. 确认改造范围
- [ ] 确认 MCP Server 名称和路径
- [ ] 确认是否需要多账号访问能力
- [ ] 备份原始代码（git commit）

### 2. 环境准备
```bash
# 1. 安装依赖
cd src/<mcp-server-name>
pip install sqlalchemy cryptography psycopg2-binary

# 2. 配置环境变量（开发环境）
cat > .env << EOF
DATABASE_URL=postgresql://user:pass@localhost:5432/costq
ENCRYPTION_KEY=<base64-encoded-fernet-key>
AWS_REGION=us-east-1
FASTMCP_TRANSPORT=stdio
EOF
```

---

## 🔧 改造步骤

### Step 1: 复制凭证提取服务层（✅ 可直接复制）

#### 1.1 创建目录并复制
```bash
cd src/<mcp-server-name>/

# ✅ 直接复制整个目录（完全通用，无需修改）
cp -r ../billing-cost-management-mcp-server/cred_extract_services ./
```

**为什么可以直接复制？**
- ✅ 完全自包含，不依赖项目特定代码
- ✅ 没有硬编码的项目名称
- ✅ 所有配置从环境变量读取
- ✅ 可以在任何 MCP Server 中直接使用

**包含的文件**：
```
cred_extract_services/
├── __init__.py           # 公共接口导出
├── aws_client.py         # STS AssumeRole（通用）
├── context_manager.py    # 环境变量管理（通用）
├── credential_extractor.py  # 凭证提取核心（通用）
├── crypto.py             # AKSK 解密（通用）
├── database.py           # 数据库查询（通用）
└── exceptions.py         # 自定义异常（通用）
```

**验证**：
```bash
# 测试导入
python -c "from cred_extract_services import extract_aws_credentials; print('✅ 导入成功')"
```

---

### Step 2: 创建统一入口 (entrypoint.py)（⚠️ 需要修改）

#### 2.1 复制并修改
```bash
# ⚠️ 复制模板后需要修改
cp ../billing-cost-management-mcp-server/entrypoint.py ./
```

**⚠️ 必须修改的地方**：只有 1 处！

```python
# 在 main() 函数中，修改导入路径
# 原始（billing-cost-management 专用）：
from awslabs.billing_cost_management_mcp_server.server import mcp, setup

# 修改为（你的项目）：
from awslabs.<your_package_name>_mcp_server.server import mcp, setup
```

**示例**：
```python
# CloudWatch MCP Server
from awslabs.cloudwatch_mcp_server.server import mcp, setup

# S3 MCP Server
from awslabs.s3_mcp_server.server import mcp, setup

# DynamoDB MCP Server
from awslabs.dynamodb_mcp_server.server import mcp, setup
```

---

#### 2.2 完整代码模板（可选，手动创建）

**如果你想手动创建 entrypoint.py，使用以下模板**：
```python
"""凭证提取服务主入口

提供统一的凭证提取和上下文设置接口。
"""

import asyncio
import logging
import os
import sys

from cred_extract_services.context_manager import set_aws_credentials
from cred_extract_services.credential_extractor import extract_aws_credentials
from cred_extract_services.exceptions import (
    AccountNotFoundError,
    AssumeRoleError,
    CredentialDecryptionError,
    CredentialExtractionError,
    DatabaseConnectionError,
)

logger = logging.getLogger(__name__)

# 导出异常类
__all__ = [
    "_setup_account_context",
    "CredentialExtractionError",
    "AccountNotFoundError",
    "CredentialDecryptionError",
    "AssumeRoleError",
    "DatabaseConnectionError",
]


async def _setup_account_context(target_account_id: str) -> dict[str, str]:
    """设置 AWS 凭证上下文

    Args:
        target_account_id: AWS 账号 ID

    Returns:
        凭证信息字典（已脱敏）

    Raises:
        AccountNotFoundError: 账号不存在
        CredentialDecryptionError: 凭证解密失败
        AssumeRoleError: AssumeRole 失败
        DatabaseConnectionError: 数据库连接失败
    """
    logger.info("开始设置 AWS 凭证上下文")

    # 1. 提取凭证
    credentials = await extract_aws_credentials(target_account_id)

    # 2. 设置环境变量
    set_aws_credentials(
        access_key_id=credentials["access_key_id"],
        secret_access_key=credentials["secret_access_key"],
        session_token=credentials.get("session_token"),
        region=credentials["region"],
    )

    # 3. 返回脱敏信息
    cred_info = {
        "account_id": credentials["account_id"],
        "account_alias": credentials.get("alias", "Unknown"),
        "auth_type": credentials["auth_type"],
        "region": credentials["region"],
    }

    logger.info(f"✅ AWS 凭证上下文设置完成: {cred_info}")
    return cred_info


def main():
    """启动 MCP Server"""
    # 延迟导入，避免循环依赖
    from awslabs.<mcp_server_package>.server import mcp, setup

    transport = os.environ.get("FASTMCP_TRANSPORT", "streamable-http")
    host = os.environ.get("FASTMCP_HOST", "0.0.0.0")
    port = int(os.environ.get("FASTMCP_PORT", "8000"))
    stateless = os.environ.get("FASTMCP_STATELESS_HTTP", "true").lower() == "true"

    logger.info(f"🚀 启动 MCP Server: transport={transport}")

    asyncio.run(setup())

    if transport == "stdio":
        mcp.run(transport=transport)
    else:
        mcp.run(transport=transport, host=host, port=port, stateless_http=stateless)


if __name__ == "__main__":
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    )
    main()
```

**注意**：
- 将 `<mcp_server_package>` 替换为实际的包名（如 `billing_cost_management_mcp_server`）

---

### Step 3: 修改所有 Tool 函数

#### 3.1 定位所有 Tool 文件
```bash
# 查找所有 tool 文件
find awslabs -name "*_tools.py" -type f
```

#### 3.2 参数顺序规则 ⚠️ 重要

**🎯 核心规则：`target_account_id` 必须放在所有必需参数之后、其他可选参数之前**

```python
# ✅ 正确的参数顺序
async def tool_function(
    ctx: Context,                             # 1. MCP Context（必需）
    required_param: str,                      # 2. 业务必需参数
    target_account_id: Optional[str] = None,  # 3. ⭐ target_account_id（第一个可选参数）
    optional_param: Optional[int] = None,     # 4. 其他可选参数
):

# ❌ 错误示例 1：target_account_id 在必需参数之前
async def tool_function(
    ctx: Context,
    target_account_id: Optional[str] = None,  # ❌ 错误！
    required_param: str,                      # Python 不允许必需参数在可选参数后面
):

# ❌ 错误示例 2：target_account_id 在其他可选参数之后
async def tool_function(
    ctx: Context,
    required_param: str,
    optional_param: Optional[int] = None,
    target_account_id: Optional[str] = None,  # ❌ 不规范，应该放在第一个
):
```

---

#### 3.3 标准改造模板

**改造前**：
```python
@server.tool(name="tool-name", description="...")
async def tool_function(
    ctx: Context,
    param1: str,                     # 必需参数
    param2: Optional[int] = None,    # 可选参数
) -> Dict[str, Any]:
    try:
        # 业务逻辑
        client = create_aws_client("service", "region")
        response = client.some_api_call(param1=param1)
        return format_response("success", response)
    except Exception as e:
        return await handle_aws_error(ctx, e, "operation", "Service")
```

**改造后**：
```python
# 1. 导入异常类（文件顶部）
from entrypoint import (
    AccountNotFoundError,
    CredentialDecryptionError,
    AssumeRoleError,
    DatabaseConnectionError,
)

@server.tool(name="tool-name", description="...")
async def tool_function(
    ctx: Context,
    param1: str,                              # 必需参数（保持原位置）
    target_account_id: Optional[str] = None,  # ✅ 新增：第一个可选参数
    param2: Optional[int] = None,             # 可选参数（保持原位置）
) -> Dict[str, Any]:
    try:
        # ===== 账号上下文初始化 =====
        if target_account_id:
            from entrypoint import _setup_account_context
            await _setup_account_context(target_account_id)

        # ===== 原有逻辑（完全不变）=====
        client = create_aws_client("service", "region")
        response = client.some_api_call(param1=param1)
        return format_response("success", response)

    # ===== 异常处理 =====
    except AccountNotFoundError:
        return format_response('error', {'error_type': 'account_not_found'},
                               'Account not found. Please check the account ID.')
    except CredentialDecryptionError:
        return format_response('error', {'error_type': 'credential_error'},
                               'Failed to decrypt credentials. Please contact administrator.')
    except AssumeRoleError:
        return format_response('error', {'error_type': 'assume_role_error'},
                               'Failed to assume role. Please check IAM role configuration.')
    except DatabaseConnectionError:
        return format_response('error', {'error_type': 'database_error'},
                               'Database connection failed. Please try again later.')
    except Exception as e:
        return await handle_aws_error(ctx, e, "operation", "Service")
```

#### 3.3 批量修改脚本（可选）
**创建 Python 脚本**：
```python
# scripts/add_account_context.py
import re
import sys
from pathlib import Path

def add_account_context_to_tool(file_path: Path):
    """给 tool 函数添加账号上下文支持"""
    content = file_path.read_text()

    # 1. 添加导入（如果不存在）
    import_block = """
# Import account context exceptions
from entrypoint import (
    AccountNotFoundError,
    CredentialDecryptionError,
    AssumeRoleError,
    DatabaseConnectionError,
)
"""
    if "from entrypoint import" not in content:
        # 在最后一个 import 后添加
        lines = content.split('\n')
        last_import_idx = 0
        for i, line in enumerate(lines):
            if line.startswith('import ') or line.startswith('from '):
                last_import_idx = i
        lines.insert(last_import_idx + 1, import_block)
        content = '\n'.join(lines)

    # 2. 修改函数签名（添加 target_account_id）
    # 模式：async def function_name(ctx: Context, ...
    pattern = r'(async def \w+\(\s*ctx:\s*Context,)'
    replacement = r'\1\n    target_account_id: Optional[str] = None,'
    content = re.sub(pattern, replacement, content)

    # 3. 添加账号上下文初始化代码
    # 在 try: 后添加
    context_init = """
        # ===== Account context initialization =====
        if target_account_id:
            from entrypoint import _setup_account_context
            await _setup_account_context(target_account_id)

        # ===== Original logic (unchanged) =====
"""
    content = content.replace("    try:\n", f"    try:\n{context_init}")

    # 4. 添加异常处理
    exception_block = """
    # ===== Exception handling =====
    except AccountNotFoundError:
        return format_response('error', {'error_type': 'account_not_found'},
                               'Account not found. Please check the account ID.')
    except CredentialDecryptionError:
        return format_response('error', {'error_type': 'credential_error'},
                               'Failed to decrypt credentials. Please contact administrator.')
    except AssumeRoleError:
        return format_response('error', {'error_type': 'assume_role_error'},
                               'Failed to assume role. Please check IAM role configuration.')
    except DatabaseConnectionError:
        return format_response('error', {'error_type': 'database_error'},
                               'Database connection failed. Please try again later.')
"""
    # 在最后的 except Exception 前添加
    content = re.sub(
        r'(\s+except Exception as e:)',
        exception_block + r'\1',
        content
    )

    # 保存修改
    file_path.write_text(content)
    print(f"✅ Modified: {file_path}")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python add_account_context.py <tool_file.py>")
        sys.exit(1)

    for file_path in sys.argv[1:]:
        add_account_context_to_tool(Path(file_path))
```

**使用脚本**：
```bash
# 修改单个文件
python scripts/add_account_context.py awslabs/.../tools/example_tools.py

# 批量修改
find awslabs -name "*_tools.py" -exec python scripts/add_account_context.py {} \;
```

**⚠️ 警告**：
- 脚本可能无法处理所有边界情况
- **必须人工审查每个修改**
- 建议先在测试文件上验证

---

### Step 4: 更新 Tool 描述文档

#### 4.1 添加参数说明
在每个 tool 的 `description` 中添加：
```python
@server.tool(
    name="tool-name",
    description="""Tool description...

    Args:
        ctx: The MCP context object
        target_account_id: Target AWS account ID. If not provided, use default credentials.
        # ... 其他参数

    Returns:
        Dict containing the response data
    """
)
```

#### 4.2 添加使用示例
```markdown
## 使用示例

### 1. 使用默认凭证
```json
{
  "tool": "tool-name",
  "params": {
    "param1": "value1"
  }
}
```

### 2. 使用目标账号凭证
```json
{
  "tool": "tool-name",
  "params": {
    "target_account_id": "123456789012",
    "param1": "value1"
  }
}
```
```

---

### Step 4.5: 检查并统一日志库（重要）⚠️

#### 4.5.1 为什么需要统一日志库？

**问题**：混用 `loguru` 和 `logging` 会导致：
- ❌ **OpenTelemetry 无法捕获 loguru 日志**
- ❌ **缺少 trace ID 和 span ID**（无法追踪请求链路）
- ❌ **CloudWatch 日志不完整**（loguru 的日志只通过 stderr 输出，没有结构化信息）
- ❌ **日志格式不一致**（难以统一分析）

**检查方法**：
```bash
# 查找项目中是否使用 loguru
cd src/<mcp-server-name>/
grep -r "from loguru" awslabs/ --include="*.py"

# 如果找到，需要迁移到 logging
```

---

#### 4.5.2 迁移步骤

**Step 1: 修改导入语句**
```python
# 修改前（tools.py 或其他文件）
from loguru import logger

# 修改后
import logging
logger = logging.getLogger(__name__)
```

**Step 2: 移除 loguru 依赖**
```toml
# pyproject.toml
[project]
dependencies = [
    "boto3>=1.38.22",
    # "loguru>=0.7.0",  # ❌ 删除此行
    "mcp[cli]>=1.23.0",
    "pydantic>=2.10.6",
]
```

**Step 3: 更新 FastMCP 依赖声明**（如果有）
```python
# server.py
mcp = FastMCP(
    name='awslabs.xxx-mcp-server',
    dependencies=[
        'boto3',
        'botocore',
        'pydantic',
        # 'loguru',  # ❌ 删除此行
    ],
    ...
)
```

**Step 4: 验证修改**
```bash
# 确认没有 loguru 引用
grep -r "loguru" . --include="*.py" --include="*.toml" || echo "✅ 没有找到 loguru 引用"

# 语法检查
python3 -m py_compile awslabs/**/*.py
```

---

#### 4.5.3 日志方法兼容性

**好消息**：`logging` 和 `loguru` 的基础 API 完全兼容，无需修改日志调用代码！

```python
# 以下代码在两个库中都可以正常工作
logger.info(f"消息: {变量}")
logger.error(f"错误: {str(e)}")
logger.warning(f"警告: {内容}")
logger.debug(f"调试: {数据}")
```

**注意**：如果使用了 loguru 特有功能，需要调整：
```python
# ❌ loguru 特有语法（需要改）
logger.opt(exception=True).error("错误")
logger.bind(request_id=123).info("消息")
logger.catch()(my_function)

# ✅ logging 等效写法
logger.error("错误", exc_info=True)
logger.info("消息", extra={"request_id": 123})
# 使用标准异常处理替代 @logger.catch()
```

---

#### 4.5.4 迁移后的优势

| 改进项 | loguru | logging | 提升 |
|--------|--------|---------|------|
| **OpenTelemetry 集成** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **Trace ID** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **Span ID** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **CloudWatch 完整性** | ⚠️ 部分 | ✅ 完整 | ⭐⭐⭐⭐ |
| **请求链路追踪** | ❌ | ✅ | ⭐⭐⭐⭐⭐ |
| **日志系统统一** | ❌ | ✅ | ⭐⭐⭐⭐ |

**CloudWatch 日志对比**：
```json
// ❌ loguru（缺少追踪信息）
"2026-01-19 12:00:00.123 | INFO | tools:lookup_events:180 - 开始查询"

// ✅ logging + OpenTelemetry（完整追踪）
{
  "timestamp": "2026-01-19T12:00:00.123Z",
  "level": "INFO",
  "logger": "awslabs.xxx_mcp_server.tools",
  "message": "开始查询",
  "trace_id": "1-abc123",  // ✅ 可追踪请求
  "span_id": "xyz789"      // ✅ 可追踪函数调用
}
```

---

#### 4.5.5 快速迁移检查清单

- [ ] 查找所有 `from loguru import` 引用
- [ ] 替换为 `import logging` + `logger = logging.getLogger(__name__)`
- [ ] 从 `pyproject.toml` 移除 `loguru` 依赖
- [ ] 从 `server.py` FastMCP 依赖移除 `loguru`
- [ ] 检查是否使用了 loguru 特有语法（需要调整）
- [ ] 验证语法和导入正确性
- [ ] 重新构建镜像

**参考文档**：`costq/docs/20260118_CloudTrail_MCP问题/20250119_loguru_迁移到_logging.md`

---

### Step 5: 修改 server.py (可选)

**如果需要自定义启动逻辑**，修改 `server.py`：
```python
# awslabs/<mcp_server_package>/server.py

#!/usr/bin/env python3
import asyncio
import os
import sys

if __name__ == '__main__':
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(os.path.dirname(current_dir))
    if parent_dir not in sys.path:
        sys.path.insert(0, parent_dir)

from fastmcp import FastMCP

mcp = FastMCP(
    name='<mcp-server-name>',
    instructions="""Server instructions..."""
)

async def setup():
    """Initialize the MCP server"""
    # 导入并注册 tools
    from .tools import tool1_server, tool2_server
    await mcp.import_server(tool1_server)
    await mcp.import_server(tool2_server)

    logger.info('MCP Server initialized successfully')

def main():
    """Main entry point"""
    asyncio.run(setup())
    mcp.run()

if __name__ == '__main__':
    main()
```

**注意**：
- 通常不需要修改 `server.py`
- 启动逻辑已经在 `entrypoint.py` 中实现

---

### Step 5.5: 新改造方案 - 直接在 server.py 配置（适用于 mcp.server.fastmcp）

**适用场景**：
- 使用 `mcp.server.fastmcp`（MCP SDK 内置）而非 `fastmcp` 独立包
- 只需要 AgentCore Runtime 部署（不需要复杂的环境变量配置）
- 不需要 entrypoint.py 的额外逻辑

#### 5.5.1 改造方案

**修改 server.py 的 FastMCP 初始化**：
```python
# 原始代码
from mcp.server.fastmcp import FastMCP

mcp = FastMCP(
    'awslabs.cloudtrail-mcp-server',
    instructions='...',
    dependencies=[...]
)

def main():
    mcp.run()
```

**改造后**：
```python
from mcp.server.fastmcp import FastMCP

# Create FastMCP server for AgentCore Runtime
# AgentCore requires stateless HTTP servers on 0.0.0.0:8000/mcp
mcp = FastMCP(
    name='awslabs.cloudtrail-mcp-server',  # 使用 name= 参数
    instructions='...',
    dependencies=[...],
    host="0.0.0.0",           # ← 新增：监听地址
    stateless_http=True       # ← 新增：无状态 HTTP 模式
)

# 注册工具（原有逻辑不变）
cloudtrail_tools = CloudTrailTools()
cloudtrail_tools.register(mcp)

def main():
    """Run the MCP server with streamable HTTP transport for AgentCore."""
    # AgentCore Runtime expects servers to run with streamable-http transport
    mcp.run(transport="streamable-http")  # ← 明确指定 transport

if __name__ == '__main__':
    main()
```

#### 5.5.2 关键修改点

| 修改项 | 原始代码 | 改造后 |
|--------|---------|--------|
| **FastMCP 初始化** | `FastMCP('name', ...)` | `FastMCP(name='name', host="0.0.0.0", stateless_http=True)` |
| **main() 函数** | `mcp.run()` | `mcp.run(transport="streamable-http")` |
| **依赖** | 无需 entrypoint.py | 如需多账号支持，仍需 entrypoint.py |

#### 5.5.3 优势对比

| 方案 | fastmcp 独立包 + entrypoint.py | mcp.server.fastmcp + server.py 配置 |
|------|-------------------------------|-----------------------------------|
| **配置位置** | entrypoint.py 读取环境变量 | server.py 初始化时传参 |
| **代码复杂度** | 需要额外的 entrypoint.py | 直接在 server.py 配置 |
| **环境变量** | 需要 FASTMCP_* 环境变量 | 不依赖环境变量 |
| **run() 调用** | `mcp.run(transport, host, port, stateless_http)` | `mcp.run(transport="streamable-http")` |
| **适用项目** | billing-cost-management | cloudtrail, cost-explorer 等 |
| **多账号支持** | ✅ 内置支持 | ⚠️ 仍需 entrypoint.py |

#### 5.5.4 何时使用此方案

**推荐使用场景**：
1. ✅ 项目已使用 `mcp.server.fastmcp`（查看 pyproject.toml 依赖）
2. ✅ 只需要 AgentCore Runtime 部署（不需要本地 stdio 测试）
3. ✅ 不需要复杂的环境变量配置逻辑
4. ✅ 代码简洁优先

**不推荐使用场景**：
1. ❌ 项目已使用 `fastmcp` 独立包（应继续使用原方案）
2. ❌ 需要灵活的环境变量配置（开发/测试/生产环境切换）
3. ❌ 需要复杂的启动前初始化逻辑

#### 5.5.5 验证方法

**构建并启动容器后，检查日志**：
```bash
docker logs <container-name> 2>&1 | grep "Uvicorn running"
```

**预期输出**：
```
INFO:     Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)
```

✅ **成功标志**：绑定到 `0.0.0.0:8000`（而不是 `127.0.0.1:8000`）

**测试 HTTP 连接**：
```bash
curl -s -o /dev/null -w "HTTP 状态码: %{http_code}\n" http://localhost:8000/
# 预期返回：404（正常，因为访问根路径）
```

#### 5.5.6 Dockerfile 修改（如果不需要多账号功能）

**原始 Dockerfile-AgentCore-Runtime**：
```dockerfile
# 复制 entrypoint.py
COPY --chown=app:app ./entrypoint.py /app/entrypoint.py

# 启动 entrypoint
CMD ["opentelemetry-instrument", "python", "-m", "entrypoint"]
```

**简化版（不需要多账号）**：
```dockerfile
# 不需要复制 entrypoint.py 和 cred_extract_services

# 直接启动 MCP Server
CMD ["awslabs.cloudtrail-mcp-server"]
```

**注意**：
- 如果需要多账号支持，仍然需要 entrypoint.py 和 cred_extract_services
- 此简化方案仅适用于只做 AgentCore Runtime 部署、不需要账号切换的场景

---

## ✅ 改造验证

### 1. 单元测试
```python
# tests/test_account_context.py
import pytest
from entrypoint import _setup_account_context
from cred_extract_services.exceptions import AccountNotFoundError

@pytest.mark.asyncio
async def test_setup_account_context_aksk():
    """测试 AKSK 账号凭证设置"""
    result = await _setup_account_context("test-aksk-account-id")
    assert result["auth_type"] == "aksk"
    assert result["account_id"] is not None

@pytest.mark.asyncio
async def test_setup_account_context_iam_role():
    """测试 IAM Role 账号凭证设置"""
    result = await _setup_account_context("test-role-account-id")
    assert result["auth_type"] == "iam_role"
    assert result["account_id"] is not None

@pytest.mark.asyncio
async def test_setup_account_context_not_found():
    """测试账号不存在异常"""
    with pytest.raises(AccountNotFoundError):
        await _setup_account_context("non-existent-account")
```

### 2. 集成测试
```bash
# 启动 MCP Server (stdio 模式)
FASTMCP_TRANSPORT=stdio python entrypoint.py

# 测试调用（使用 MCP Client）
echo '{
  "jsonrpc": "2.0",
  "id": 1,
  "method": "tools/call",
  "params": {
    "name": "tool-name",
    "arguments": {
      "target_account_id": "123456789012",
      "param1": "value1"
    }
  }
}' | python entrypoint.py
```

### 3. 手动验证清单
- [ ] AKSK 账号凭证正常
- [ ] IAM Role 账号凭证正常
- [ ] External ID 正确传递
- [ ] Session Token 处理正常
- [ ] 多账号切换正常
- [ ] 默认凭证（无 target_account_id）正常
- [ ] 所有异常处理正常
- [ ] 日志不包含敏感信息

---

## 🐛 常见问题排查

### 问题 1: ModuleNotFoundError: No module named 'cred_extract_services'
**原因**：Python 路径配置问题

**解决**：
```python
# 在 entrypoint.py 中添加
import sys
import os

current_dir = os.path.dirname(os.path.abspath(__file__))
if current_dir not in sys.path:
    sys.path.insert(0, current_dir)
```

### 问题 2: DatabaseConnectionError
**原因**：数据库配置错误

**解决**：
```bash
# 检查环境变量
echo $DATABASE_URL
echo $RDS_SECRET_NAME

# 测试数据库连接
python -c "
from cred_extract_services.database import query_account
import asyncio
result = asyncio.run(query_account('test-account-id'))
print(result)
"
```

### 问题 3: CredentialDecryptionError
**原因**：加密密钥错误

**解决**：
```bash
# 检查加密密钥
echo $ENCRYPTION_KEY | base64 -d | xxd  # 应该是 32 字节

# 重新生成密钥（仅测试环境）
python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
```

### 问题 4: AssumeRoleError
**原因**：IAM Role 配置错误

**解决**：
```bash
# 检查 Role ARN 和 External ID
python -c "
from cred_extract_services.database import query_account
import asyncio
account = asyncio.run(query_account('test-role-account-id'))
print(f'Role ARN: {account[\"role_arn\"]}')
print(f'External ID: {account[\"external_id\"]}')
"

# 测试 AssumeRole
aws sts assume-role \
  --role-arn <role-arn> \
  --role-session-name test \
  --external-id <external-id>
```

### 问题 5: Session Token 残留
**原因**：环境变量未正确清理

**解决**：
```python
# 在 context_manager.py 中确保删除逻辑
if session_token:
    os.environ["AWS_SESSION_TOKEN"] = session_token
elif "AWS_SESSION_TOKEN" in os.environ:
    del os.environ["AWS_SESSION_TOKEN"]  # ✅ 关键！
```

---

## 📝 改造检查清单

### 代码修改
- [ ] 复制 `cred_extract_services/` 目录
- [ ] 创建 `entrypoint.py`
- [ ] 修改所有 tool 函数（添加 `target_account_id` 参数）
- [ ] 添加账号上下文初始化代码
- [ ] 添加异常处理
- [ ] 更新 tool 描述文档

### 测试验证
- [ ] 单元测试通过
- [ ] 集成测试通过
- [ ] 手动验证通过
- [ ] 异常处理验证通过

### 文档更新
- [ ] README 更新
- [ ] API 文档更新
- [ ] 使用示例更新

### 部署准备
- [ ] 环境变量配置
- [ ] 数据库迁移（如果需要）
- [ ] 依赖项更新 (requirements.txt / pyproject.toml)

---

## 🚀 部署上线

### 1. 更新依赖
```toml
# pyproject.toml
[project]
dependencies = [
    "fastmcp>=0.4.0",
    "boto3>=1.35.0",
    "sqlalchemy>=2.0.0",
    "psycopg2-binary>=2.9.0",
    "cryptography>=41.0.0",
    # ... 其他依赖
]
```

### 2. 配置生产环境变量
```bash
# 生产环境 (EKS)
kubectl create secret generic mcp-server-secrets \
  --from-literal=RDS_SECRET_NAME=costq/rds/postgresql \
  --from-literal=ENCRYPTION_KEY=<base64-key> \
  --from-literal=AWS_REGION=ap-northeast-1
```

### 3. 创建 Dockerfile-AgentCore-Runtime（✅ 可直接复制）

#### 3.1 直接复制
```bash
cd src/<mcp-server-name>/

# ✅ 直接复制（完全通用，无需修改）
cp ../billing-cost-management-mcp-server/Dockerfile-AgentCore-Runtime ./
```

**为什么可以直接复制？**
- ✅ 没有硬编码的项目名称
- ✅ 使用相对路径（`COPY . /app`）
- ✅ 所有配置通过环境变量
- ✅ 适用于所有使用 `pyproject.toml` + `uv` 的 MCP Server

**关键内容（已包含，无需修改）**：
- 复制 `entrypoint.py` 和 `cred_extract_services/`
- 安装额外依赖（OpenTelemetry、SQLAlchemy、psycopg2、cryptography）
- 使用 `opentelemetry-instrument` 启动
- 配置正确的环境变量

---

#### 3.2 完整 Dockerfile 模板（仅供参考）

**如果参考项目不可用，可以使用以下完整模板**：

```dockerfile
# Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
# ... (license header)

FROM public.ecr.aws/docker/library/python:3.13-alpine AS uv

WORKDIR /app

# UV 配置
ENV UV_COMPILE_BYTECODE=1 \
    UV_LINK_MODE=copy \
    UV_PYTHON_PREFERENCE=only-system \
    UV_FROZEN=true

COPY pyproject.toml uv.lock uv-requirements.txt ./

# 安装构建依赖（包含 postgresql-dev）
RUN apk update && \
    apk add --no-cache --virtual .build-deps \
    build-base gcc musl-dev libffi-dev openssl-dev cargo postgresql-dev

# 安装 Python 依赖
RUN --mount=type=cache,target=/root/.cache/uv \
    pip install --require-hashes --requirement uv-requirements.txt --no-cache-dir && \
    uv sync --python 3.13 --frozen --no-install-project --no-dev --no-editable

# 复制项目代码并安装
COPY . /app
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --python 3.13 --frozen --no-dev --no-editable

RUN mkdir -p /root/.local

# ====== Runtime 阶段 ======
FROM public.ecr.aws/docker/library/python:3.13-alpine

ENV PATH="/app/.venv/bin:$PATH" \
    PYTHONPATH="/app" \
    PYTHONUNBUFFERED=1

WORKDIR /app

# 安装运行时依赖（包含 postgresql-libs）
RUN apk update && \
    apk add --no-cache ca-certificates postgresql-libs && \
    update-ca-certificates && \
    addgroup -S app && \
    adduser -S app -G app -h /app

# 复制虚拟环境
COPY --from=uv --chown=app:app /app/.venv /app/.venv

# ✅ 关键：复制 entrypoint.py 和 cred_extract_services
COPY --chown=app:app ./entrypoint.py /app/entrypoint.py
COPY --chown=app:app ./cred_extract_services /app/cred_extract_services

# 复制健康检查脚本
COPY ./docker-healthcheck.sh /usr/local/bin/docker-healthcheck.sh

# ====== Runtime 专用配置 ======
EXPOSE 9000 8000 8080

ENV UV_SYSTEM_PYTHON=1 \
    UV_NO_PROGRESS=1 \
    DOCKER_CONTAINER=1 \
    AWS_REGION=ap-northeast-1 \
    AWS_DEFAULT_REGION=ap-northeast-1 \
    FASTMCP_HOST=0.0.0.0 \
    FASTMCP_PORT=8000 \
    FASTMCP_STATELESS_HTTP=true \
    FASTMCP_TRANSPORT=streamable-http

# ✅ 关键：安装额外依赖
RUN pip install --no-cache-dir uv && \
    uv pip install --python /app/.venv/bin/python \
    aws-opentelemetry-distro==0.12.2 \
    sqlalchemy>=2.0.0 \
    psycopg2-binary>=2.9.0 \
    cryptography>=41.0.0

USER app

HEALTHCHECK --interval=60s --timeout=10s --start-period=10s --retries=3 \
    CMD ["docker-healthcheck.sh"]

# ✅ 关键：使用 opentelemetry-instrument 启动
CMD ["opentelemetry-instrument", "python", "-m", "entrypoint"]
```

#### 3.3 关键修改点检查清单

- [ ] 复制 `entrypoint.py` 和 `cred_extract_services/`
- [ ] 安装 4 个额外依赖（OpenTelemetry、SQLAlchemy、psycopg2、cryptography）
- [ ] 使用 `opentelemetry-instrument python -m entrypoint` 启动
- [ ] 暴露端口 8000、8080、9000
- [ ] 设置正确的环境变量（MCP_* 和 AWS_*）
- [ ] 包含 `postgresql-dev`（构建阶段）和 `postgresql-libs`（运行阶段）

---

### 4. 部署脚本使用

#### 🎯 方式 1：直接使用通用脚本（推荐⭐）

**最简单的方式**，无需创建新文件，直接传参使用：

```bash
# 从项目根目录执行
bash costq/scripts/build_and_push_template.sh <mcp-server-name>

# 示例
bash costq/scripts/build_and_push_template.sh cloudtrail-mcp-server
bash costq/scripts/build_and_push_template.sh cloudwatch-mcp-server
bash costq/scripts/build_and_push_template.sh billing-cost-management-mcp-server
```

**优点**：
- ✅ 无需创建新文件
- ✅ 无需修改配置
- ✅ 一条命令完成部署
- ✅ 适合临时部署和快速测试

**脚本会自动**：
1. 验证参数（如果未提供会显示用法说明）
2. 登录 ECR
3. 构建 ARM64 镜像
4. 打标签（`latest` + 时间戳）
5. 推送到 ECR

---

#### 🔧 方式 2：创建专用脚本（可选）

**适用场景**：需要自定义 AWS Profile/Region/Account 或团队共享配置

##### 4.1 复制脚本模板
```bash
cd costq/scripts/

# 复制通用模板
cp build_and_push_template.sh 01-build_and_push_<mcp-server-name>.sh

# 添加执行权限
chmod +x 01-build_and_push_<mcp-server-name>.sh
```

##### 4.2 修改配置
编辑脚本，修改以下变量：
```bash
# 方式 1：修改 MCP_SERVER_NAME（移除参数解析逻辑，直接硬编码）
MCP_SERVER_NAME="cloudwatch-mcp-server"

# 方式 2：保留参数功能，只修改默认值
MCP_SERVER_NAME="${MCP_SERVER_NAME:-cloudwatch-mcp-server}"

# 可选：修改 AWS 配置
AWS_PROFILE="3532"                   # AWS CLI Profile
AWS_REGION="ap-northeast-1"          # AWS 区域
AWS_ACCOUNT="000451883532"           # AWS 账号 ID
```

##### 4.3 执行部署
```bash
# 从项目根目录执行
bash costq/scripts/01-build_and_push_<mcp-server-name>.sh

# 或者使用参数覆盖（如果保留了参数功能）
bash costq/scripts/01-build_and_push_<mcp-server-name>.sh another-server
```

**脚本会自动执行**：
1. ✅ 登录 ECR
2. ✅ 构建 ARM64 镜像
3. ✅ 打标签（`latest` + 时间戳）
4. ✅ 推送到 ECR

**预期输出**：
```
============================================================
🚀 构建并推送 MCP Server 镜像
============================================================
MCP Server: cloudwatch-mcp-server
ECR 仓库: 000451883532.dkr.ecr.ap-northeast-1.amazonaws.com/awslabs-mcp/cloudwatch-mcp-server
镜像标签: latest, v20260117-123456
平台: linux/arm64
============================================================

🔐 Step 1: 登录 ECR...
✅ ECR 登录成功

🔨 Step 2: 构建 ARM64 Docker 镜像...
✅ 镜像构建成功

🏷️  Step 3: 打标签...
✅ 标签已创建: latest, v20260117-123456

📤 Step 4: 推送镜像到 ECR...
✅ 镜像推送成功

============================================================
✅ 镜像部署完成!
============================================================
```

---

### 5. 手动部署（不推荐）

**如果不使用脚本，可以手动执行以下步骤**：

#### 5.1 构建 ARM64 镜像
```bash
cd src/<mcp-server-name>/

# 构建镜像（使用 buildx 支持多平台）
docker buildx build \
  --platform linux/arm64 \
  -f Dockerfile-AgentCore-Runtime \
  -t <ecr-url>/<mcp-server-name>:latest \
  --load \
  .
```

#### 5.2 推送到 ECR
```bash
# 登录 ECR
aws ecr get-login-password --region ap-northeast-1 --profile 3532 | \
  docker login --username AWS --password-stdin <account-id>.dkr.ecr.ap-northeast-1.amazonaws.com

# 打标签
docker tag <mcp-server-name>:latest <ecr-url>/<mcp-server-name>:latest

# 推送镜像
docker push <ecr-url>/<mcp-server-name>:latest
```

---

### 6. 更新 Runtime 和验证

#### 6.1 更新 Runtime
```bash
# 使用 AWS CLI 更新
aws bedrock-agentcore-control update-runtime \
  --profile 3532 \
  --region ap-northeast-1 \
  --runtime-identifier <runtime-id> \
  --container-image <ecr-url>/<mcp-server-name>:latest
```

#### 6.2 刷新 Gateway
⚠️ **重要**：按照 DEEPV.md 说明，需要手动刷新 Gateway

#### 6.3 验证部署

### 4. 监控验证
```bash
# 查看日志
kubectl logs -f deployment/mcp-server -n costq-mcp

# 验证健康检查
curl http://mcp-server:8000/health

# 测试 API
curl -X POST http://mcp-server:8000/mcp \
  -H "Content-Type: application/json" \
  -d '{"method": "tools/list"}'
```

---

## 📚 参考资料

- [改造需求分析](./01_改造需求分析.md)
- [Skill vs Memory 方案对比](./03_Skill_vs_Memory方案对比.md)
- [改造自动化方案](./04_改造自动化方案.md)
